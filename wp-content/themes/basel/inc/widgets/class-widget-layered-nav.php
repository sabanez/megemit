	...
	class BASEL_Widget_Layered_Nav extends WPH_Widget {
		...
		function __construct() {
			...
			if ( isset( $GLOBALS['WC_Brands'] ) ) {
				$attribute_array[ _x( 'Product brands', 'Product Attribute', 'woocommerce' ) ] = 'product_brand';
			}
			...
			$args['fields'] = array(
				...
				array(
					...
					'type'    => 'dropdown',
					'std'     => '',
					'name'   => esc_html__( 'Source', 'woocommerce' ),
		...
		function widget( $args, $instance )	{
			...
			if ( 'product_brand' === $instance['attribute'] && taxonomy_exists( 'product_brand' ) ) {
		...
		protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
			...
			if ( $taxonomy !== $this->get_current_taxonomy() ) {
				...
				foreach ( $terms as $term ) {
					...
					$option_is_set = in_array( $term->slug, $current_values ) || in_array( $term->term_id, $current_values );
					...
					echo '<option value="' . esc_attr( $slug ) . '" ' . selected( $option_is_set, true, false ) . '>' . esc_html( $term->name ) . '</option>';
				}
		...
		protected function get_page_base_url( $taxonomy ) {
			...
			if ( $_chosen_attributes = $this->get_chosen_attributes() ) { // phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.Found, WordPress.CodeAnalysis.AssignmentInCondition.Found
		...
		protected function layered_nav_list( $terms, $taxonomy, $query_type, $instance ) {
			...
			$is_brand = ( basel_get_opt( 'brands_attribute' ) == $taxonomy ) || ( 'product_brand' === $taxonomy );
			...
			foreach ( $terms as $term ) {
				...
				if ( 'product_brand' === $taxonomy && ! in_array( $term->term_id, $current_filter ) ) {
					$current_filter[] = $term->term_id;
				} elseif ( ! in_array( $term->slug, $current_filter ) ) {
				...
				foreach ( $current_filter as $key => $value ) {
					...
					if ( $value === $this->get_current_term_slug() || 'product_brand' === $taxonomy && (int) $value === $this->get_current_term_id() ) {
						unset( $current_filter[ $key ] );
					}

					// Exclude self so filter can be unset on click.
					if ( $option_is_set && ( $value === $term->slug || 'product_brand' === $taxonomy && (int) $value === $term->term_id ) ) {
						unset( $current_filter[ $key ] );
					}
				}
				...
				if ( 'product_brand' === $taxonomy && ! $swatch_image ) {
					$thumbnail_id = get_term_meta( $term->term_id, 'thumbnail_id', true );

					if ( $thumbnail_id ) {
						$swatch_image = wp_get_attachment_url( $thumbnail_id );
					}
				}
