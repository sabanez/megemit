...
class Manager extends Singleton {
	 ...
	 *
	 * @var string $transient_free_gifts_ids .
	 */
	public $transient_free_gifts_ids = 'basel_transient_free_gifts_ids';

	/**
	 * Transient name for 'Single free gifts rule'. Has a dynamic part at the end of the name '_${free_gifts_post_id}'.
	 *
	 * @var string $transient_free_gifts_rule .
	 */
	public $transient_free_gifts_rule = 'basel_transient_free_gifts_rule';

	/**
	 * Transient name for 'All gifts rules'.
	 *
	 * @var string $transient_free_gifts_all_rules .
	 */
	public $transient_free_gifts_all_rules = 'basel_transient_free_gifts_all_rules';
	...
	public function get_single_post_rules( $id = '' ) {
		...
		$cache = get_transient( $this->transient_free_gifts_rule . '_' . $id );
		...
		set_transient( $this->transient_free_gifts_rule . '_' . $id, $current_meta_boxes );
	...
	public function get_all_rule_posts_ids() {
		$cache = get_transient( $this->transient_free_gifts_ids );
		...
		$all_free_gifts_post_ids = get_posts(
			array(
				'fields'         => 'ids',
				'posts_per_page' => apply_filters( 'basel_free_gifts_rule_limit', 100 ),
				'post_type'      => 'basel_woo_free_gifts',
				'post_status'    => 'publish',
			)
		);

		set_transient( $this->transient_free_gifts_ids, $all_free_gifts_post_ids );
	...
	public function get_rules( $type = 'all' ) {
		$all_rules = get_transient( $this->transient_free_gifts_all_rules );
		...
		set_transient( $this->transient_free_gifts_all_rules, $all_rules );
	...
	public function check_free_gifts_totals( $gift_rule, $cart_price = false ) {
		if ( false === $cart_price ) {
			...
			$cart_price = isset( $gift_rule['free_gifts_cart_price_type'] ) && 'subtotal' === $gift_rule['free_gifts_cart_price_type'] ? $totals['subtotal'] : $totals['total'];
		}
	...
	public function get_condition_priority( $type ) {
		...
		return apply_filters( 'basel_condition_priority', $priority, $type );
	}
	...
	public function check_is_gift_in_cart( $product_id, $cart_object = '' ) {
		...
		$cart_items = array_filter(
			...
			function ( $cart_item ) {
				return array_key_exists( 'basel_is_free_gift', $cart_item );
			}
		);
	...
	public function get_gifts_in_cart_count() {
		$gifts = array_filter(
			WC()->cart->get_cart(),
			function ( $cart_item ) {
				return isset( $cart_item['basel_is_free_gift'] );
			}
		);
	...
	public function set_notices() {
		$this->notices = apply_filters(
			'basel_free_gifts_notices',
			array(
				'added_successfully' => esc_html__( 'Gift product added successfully.', 'basel' ),
				'already_added'      => esc_html__( 'The gift has already been added to the cart.', 'basel' ),
				'free_gifts_limit'   => esc_html__( 'There is already a maximum number of gifts in the cart.', 'basel' ),
			)
		);
	}
