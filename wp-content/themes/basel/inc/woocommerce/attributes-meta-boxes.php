...
if( ! function_exists( 'basel_wc_attribute_update' ) ) {
	...
	function basel_wc_attribute_update( $attribute_id, $attribute, $old_attribute_name ) {
		update_option( 'basel_pa_' . $attribute['attribute_name'] . '_swatch_size', sanitize_text_field( $_POST['attribute_swatch_size'] ), false );
		
		$attribute_show_on_product = isset( $_POST['attribute_show_on_product'] ) ? $_POST['attribute_show_on_product'] : '';
		update_option( 'basel_pa_' . $attribute['attribute_name'] . '_show_on_product', sanitize_text_field( $attribute_show_on_product ), false );
