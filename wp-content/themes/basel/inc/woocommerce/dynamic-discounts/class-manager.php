...
class Manager extends Singleton {
	 ...
	 *
	 * @var string $transient_discounts_ids .
	 */
	public $transient_discounts_ids = 'basel_all_discounts_post_ids';
	...
	public function get_all_discounts_post_ids() {
		$cache = get_transient( $this->transient_discounts_ids );
		...
		$all_discounts_post_ids = get_posts(
			array(
				'fields'         => 'ids',
				'posts_per_page' => -1,
				'post_type'      => 'basel_woo_discounts',
			)
		);

		set_transient( $this->transient_discounts_ids, $all_discounts_post_ids );
	...
	public function sort_by_priority( $a, $b ) {
		return $b['basel_discount_priority'] <=> $a['basel_discount_priority'];
	}
	...
	public function check_discount_condition( $discount_rules, $product ) {
		...
		foreach ( $conditions as $discount_id => $condition ) {
			$conditions[ $discount_id ]['basel_discount_priority'] = $this->get_condition_priority( $condition['type'] );
		}
	...
	public function get_condition_priority( $type ) {
		...
		return apply_filters( 'basel_dynamic_price_condition_priority', $priority, $type );
	}
	...
	public function get_template( $template_name, $args = array() ) {
		...
		include BASEL_THEMEROOT . '/inc/woocommerce/dynamic-discounts/templates/' . $template_name . '.php';
	}
}
