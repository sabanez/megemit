...
if ( ! function_exists( 'basel_compare_display_field' ) ) {
	...
	function basel_compare_display_field( $field_id, $product ) {
		...
		switch ( $type ) {
			...
			case 'product_brand':
			case 'attribute':
				if ( $field_id === basel_get_opt( 'brands_attribute' ) || $field_id === 'product_brand' ) {
					...
					foreach ( $brands as $brand ) {
						...
						if ( 'product_brand' === $field_id ) {
							$thumbnail_id = get_term_meta( $brand->term_id, 'thumbnail_id', true );

							if ( $thumbnail_id ) {
								$image = wp_get_attachment_image( $thumbnail_id, 'full', false, array( 'title' => $brand->name, 'alt' => $brand->name ) );
							}
						} elseif ( $image ) {
							$image = '<img src="' . esc_url( $image ) . '" title="' . esc_attr( $brand->name ) . '" alt="' . esc_attr( $brand->name ) . '" />';
						}
