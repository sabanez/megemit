...
(function($) {
        ...
        $field.select2({
            ajax             : {
                ...
                data    : function(params) {
                    return {
                        action    : 'basel_discount_conditions_query',
                        security  : baselConfig.get_new_template_nonce,
    ...
    function validate() {
        let isValid = true;
        let ruleType = $('#_basel_rule_type').val();
        let $ruleRows = $('.xts-_basel_discount_rules-field .xts-controls-wrapper > .xts-discount:not(.title)');
        let $conditionRows = $('.xts-_basel_discount_condition-field .xts-controls-wrapper > .xts-discount:not(.title)');
        let discountRulesSelector = '.xts-_basel_discount_rules-field';
        let discountConditionSelector = '.xts-_basel_discount_condition-field';
        ...
        if ( 0 === $ruleRows.length ) {
            showNotice( discountRulesSelector, basel_discounts_notice.no_quantity_range );
            isValid = false;
        }

        if ( 0 === $conditionRows.length ) {
            showNotice( discountConditionSelector, basel_discounts_notice.no_discount_condition );
            isValid = false;
        }
        ...
        $ruleRows.each((key,ruleRow) => {
            ...
            if ( key !== $ruleRows.length - 1 && priceTo >= nextPriceFrom ) {
                ...
                showNotice( discountRulesSelector, basel_discounts_notice.quantity_range_start );
                isValid = false;
            }

            if ( priceFrom > priceTo ) {
                showNotice( discountRulesSelector, basel_discounts_notice.closing_quantity );
                isValid = false;
            }

            if ( 'percentage' === type && discountPercentageValue > 100 ) {
                showNotice( discountRulesSelector, basel_discounts_notice.max_value );
                isValid = false;
            }
        });
    ...
    $(document)
        .ready( function() {
            ...
            $('.xts-_basel_discount_rules-field .xts-controls-wrapper > .xts-discount:not(.title)').each((key,ruleRow) => {
        ...
        .on('click', '.xts-_basel_discount_rules-field .xts-add-row', function() {
            let ruleType = $('#_basel_rule_type').val();
            let $ruleRows = $('.xts-_basel_discount_rules-field .xts-controls-wrapper > .xts-discount:not(.title)');
