		...
		var baselOptionsAdmin = {
			...
			tabsField: function () {
				$('.xts-tabs').each(function() {
					...
					var $tabsList = $tabsElement.find('.xts-nav-tabs > li');
					var $content =  $tabsElement.find('.xts-tab-content-wrapper > .xts-tab-content');
					...
					$tabsList.on('click', function(e) {
						...
						setTimeout(function() {
							$thisTab.siblings().removeClass('xts-active');

							$activeContent.siblings().removeClass('xts-active');
						}, animationTime);

						setTimeout(function() {
							$thisTab.addClass('xts-active');

							$activeContent.siblings().removeClass('xts-active');
							$activeContent.addClass('xts-active');
						}, animationTime);
					...
					if ( !$($tabsList[0]).hasClass( 'xts-active' ) && !$tabsElement.hasClass( 'xts-inited' ) ) {
						$($tabsList[0]).trigger( 'click' );
					}

					setTimeout(function() {
						$tabsElement.addClass( 'xts-inited' );
					}, animationTime * 2);
				});
			},
			...
			selectWithTableControl: function () {
				...
				$('.xts-active-section .xts-select_with_table-control, .xts-active-section .xts-conditions-control').each( function () {
					var $control = $(this);
			...
			conditionControl: function () {
				if ( typeof ($.fn.select2) === 'undefined' ) {
					return;
				}

				$('.xts-active-section .xts-condition-query:not(.xts-hidden) select.xts-condition-query').each((key, field) => {
				...
				$(document).on('change', '.xts-active-section select.xts-condition-type', function() {
					var $this = $(this);
				...
				function conditionQueryFieldInit( conditionType, $querySelect ) {
					...
					if ('all' === conditionType) {
						...
						$querySelect.select2({
							ajax             : {
								...
								data    : function(params) {
									return {
										action: 'basel_conditions_query',
										security: baselConfig.get_theme_settings_data_nonce,
										query_type: $querySelect.attr('data-query-type'),
										search: params.term
									};
								},
								method  : 'POST',
					...
					$querySelect.parents('.xts-controls-wrapper').find('select.xts-condition-type').each((key, type) => {
						if ( 'all' !== $(type).val() ) {
							showTitle = true;
						}
					});
		...
		return {
			init: function () {
				$(document).ready(function () {
					...
					baselOptionsAdmin.tabsField();
					baselOptionsAdmin.customFontsControl();
					baselOptionsAdmin.settingsSearch();
					baselOptionsAdmin.instagramAPI();
					baselOptionsAdmin.selectWithTableControl();
					baselOptionsAdmin.conditionControl();
				...
				$(document).on('xts_section_changed', function() {
					...
					baselOptionsAdmin.responsiveRangeControl();
				});
			},
		};
