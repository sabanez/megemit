...
if( ! function_exists( 'basel_shortcode_google_map' ) ) {
	function basel_shortcode_google_map( $atts, $content ) {
		...
		wp_enqueue_script( 'google.map.api', 'https://maps.google.com/maps/api/js?libraries=geometry&callback=baselThemeModule.googleMapsCallback&v=weekly&key=' . $google_key . '', array(), '', false );
...
if( ! function_exists( 'basel_shortcode_team_member' )) {
	function basel_shortcode_team_member($atts, $content = "") {
        ...
        if ($linkedin != '' || $twitter != '' || $facebook != '' || $instagram != '') {
	        ...
	        if ($twitter != '') {
		        $socials .= '<li class="social-x"><a href="'.esc_url( $twitter ).'" aria-label="' . esc_attr( __( sprintf( 'Social icon %s', 'X' ) , 'basel' ) ) . '"><i class="fa fa-x"></i></a></li>';
	        }
	        if ($linkedin != '') {
...
if( ! function_exists( 'basel_shortcode_info_box' )) {
	function basel_shortcode_info_box($atts, $content) {
		...
		$render_svg = apply_filters( 'basel_render_svg_with_image_tag', true ) && $no_svg_animation;
        ?>
									...
									<?php
										...
										if( substr($src, -3, 3) == 'svg' ) {
											if ( ! $render_svg ) {
												...
												echo '<div class="info-svg-wrapper" style="width: ' . $width . 'px;height: ' . $height . 'px;">';
												echo wpb_getImageBySize( array( 'attach_id' => $img_id, 'class' => 'info-icon image-' . $i ) )['thumbnail'];
												echo '</div>';
											}
										} else {
		...
		?>
			...
			<ul class="social-icons text-<?php echo esc_attr( $align ); ?> icons-design-<?php echo esc_attr( $style ); ?> icons-size-<?php echo esc_attr( $size ); ?> social-<?php echo esc_attr( $type ); ?> <?php echo esc_attr( $el_class ); ?>">
				...
				<?php if ( ( $type == 'share' && basel_get_opt('share_twitter') ) || ( $type == 'follow' && basel_get_opt( 'twitter_link' ) != '')): ?>
					<li class="social-x"><a rel="noopener noreferrer nofollow" href="<?php echo 'follow' == $type ? basel_get_opt( 'twitter_link' ) : 'https://twitter.com/share?url=' . $page_link; ?>" target="<?php echo esc_attr( $target ); ?>" class="<?php if( $tooltip == "yes" ) echo 'basel-tooltip'; ?>"><i class="fa fa-x"></i><span class="basel-social-icon-name"><?php _e('X', 'basel') ?></span></a></li>
				<?php endif ?>
				...
				<?php if ( $type == 'share' && basel_get_opt('share_tiktok') || ( $type == 'follow' && basel_get_opt( 'tiktok_link' ) != '' ) ): ?>
					<li class="social-tiktok">
						<a rel="noopener noreferrer nofollow" href="<?php echo basel_get_opt( 'tiktok_link' ); ?>" target="<?php echo esc_attr( $target ); ?>" class="<?php if( $tooltip == "yes" ) echo 'basel-tooltip'; ?>">
							<i class="fa fa-tiktok"></i>
							<span class="basel-social-icon-name">
								<?php esc_html_e( 'TikTok', 'basel' ); ?>
							</span>
						</a>
					</li>
				<?php endif ?>
		...
		if( $layout == 'carousel' ) {
			if ( ( 'auto' !== $slides_per_view_tablet && ! empty( $slides_per_view_tablet ) ) || ( 'auto' !== $slides_per_view_mobile && ! empty( $slides_per_view_mobile ) ) ) {
				$parsed_atts['custom_sizes'] = array(
					...
					'desktop_small' => $slides_per_view_tablet,
					'tablet'        => $slides_per_view_tablet,
					'mobile'        => $slides_per_view_mobile,
				);
			}

			return basel_generate_posts_slider( $parsed_atts, $products );
		}
	...
	function basel_shortcode_brands( $atts, $content = '' ) {
		...
		if ( empty( $attribute ) || ! taxonomy_exists( $attribute ) ) {
			if ( ! taxonomy_exists( 'product_brand' ) ) {
				return '<div class="basel-notice basel-info">' . esc_html__( 'You must select your brand attribute in Theme Settings -> Shop -> Brands', 'basel' ) . '</div>';
			} else {
		...
		echo '<div class="brands-items-wrapper ' . ( ( $style == 'carousel' ) ? 'owl-carousel ' : '' ) . '">';
 		...
 		if( ! is_wp_error( $brands ) && count( $brands ) > 0 ) {
			foreach ($brands as $key => $brand) {
				...
				if ( 'product_brand' === $attribute ) {
					$thumbnail_id = get_term_meta( $brand->term_id, 'thumbnail_id', true );

					if ( $thumbnail_id ) {
						$img_content = wp_get_attachment_image( $thumbnail_id, 'full', false, array( 'title' => $brand->name, 'alt' => $brand->name ) );
					}
				} elseif ( $image ) {
					$attachment_id = attachment_url_to_postid( $image );

					if ( $attachment_id ) {
						$img_content = wp_get_attachment_image( $attachment_id, 'full', false, array( 'title' => esc_attr( $brand->name ), 'alt' => esc_attr( $brand->name ) ) );
					} else {
						$img_content = apply_filters( 'basel_image', '<img src="' . esc_url( $image ) . '" title="' . esc_attr( $brand->name ) . '" alt="' . esc_attr( $brand->name ) . '" />' );
					}
				}
				...
				echo '<div class="brand-item">';
					...
					if( $style == 'list' || empty( $img_content ) ) {
