...
if ( ! function_exists( 'basel_get_owl_items_numbers' ) ) {
	function basel_get_owl_items_numbers( $slides_per_view, $post_type = false, $custom_sizes = false ) {
		...
		if ( $custom_sizes && is_array( $custom_sizes ) ) {
			if ( empty( $custom_sizes['desktop_small'] ) || 'auto' === $custom_sizes['desktop_small'] ) {
				if ( $items['desktop'] == 6 ) {
					$custom_sizes['desktop_small'] = 4;
				} else {
					$custom_sizes['desktop_small'] = ( $items['desktop'] > 1 ) ? $items['desktop'] - 1 : 1;
				}
			}

			if ( empty( $custom_sizes['tablet'] ) || 'auto' === $custom_sizes['tablet'] ) {
				$custom_sizes['tablet'] = ( $custom_sizes['desktop_small'] > 1 ) ? $custom_sizes['desktop_small'] : 1;
			}

			if ( empty( $custom_sizes['mobile'] ) || 'auto' === $custom_sizes['mobile'] ) {
				$custom_sizes['mobile'] = ( $custom_sizes['tablet'] > 2 ) ? $custom_sizes['tablet'] - 2 : 1;
			}
...
if( ! function_exists( 'basel_page_title' ) ) {
	...
	function basel_page_title() {
		...
		if( basel_woocommerce_installed() && ( is_shop() || is_product_category() || is_product_tag() || is_tax( 'product_brand' ) || is_singular( "product" ) || basel_is_product_attribute_archieve() )
		...
		?>
			<form role="search" method="get" id="searchform" class="searchform <?php echo esc_attr( $class ); ?>" action="<?php echo esc_url( home_url( '/' ) ); ?>" <?php echo ! empty( $data ) ? $data : ''; ?>>
				<div>
					...
					<button type="submit" id="searchsubmit" class="<?php echo esc_attr( function_exists( 'wc_wp_theme_get_element_class_name' ) && wc_wp_theme_get_element_class_name( 'button' ) ? wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" value="<?php echo esc_attr_x( 'Search', 'submit button', 'basel' ); ?>"><?php echo esc_html_x( 'Search', 'submit button', 'basel' ); ?></button>
				</div>
			</form>
	...
	function basel_login_form( $echo = true, $action = false, $message = false, $hidden = false, $redirect = false ) {
		...
		<form method="post" class="login woocommerce-form woocommerce-form-login <?php if ( $hidden ) echo 'hidden-form'; ?>" <?php echo ( ! empty( $action ) ) ? 'action="' . esc_url( $action ) . '"' : ''; ?> <?php if ( $hidden ) echo 'style="display:none;"'; ?>>
			...
			<p class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide form-row-username">
				<label for="username"><?php esc_html_e( 'Username or email', 'basel' ); ?>&nbsp;<span class="required" aria-hidden="true">*</span><span class="screen-reader-text"><?php esc_html_e( 'Required', 'woocommerce' ); ?></span></label>
			...
			<p class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide form-row-password">
				<label for="password"><?php esc_html_e( 'Password', 'basel' ); ?>&nbsp;<span class="required"  aria-hidden="true">*</span><span class="screen-reader-text"><?php esc_html_e( 'Required', 'woocommerce' ); ?></span></label>
...
if ( !function_exists( 'basel_get_twitts' ) ) {
	function basel_get_twitts( $args = array() ) {
		...
		if ( !isset( $args['name'] ) || !isset( $args['consumer_key'] ) || !isset( $args['consumer_secret'] ) || !isset( $args['access_token'] ) || !isset( $args['accesstoken_secret'] ) ) {
			echo '<p>You must correctly enter the X access keys</p>';
		}
		...
		if ( ! $fetchedTweets ) {
			...
			if ( $connection->http_code != 200 ) {
				echo esc_html__( 'X not return 200', 'basel' );
				return;
			}
		...
		if ( ! $fetchedTweets ) {
			echo esc_html__( 'X not return any data', 'basel' );
		}
                    ...
                    <li class="twitter-post">
						<?php if ( isset( $args['show_avatar'] ) && $args['show_avatar']): ?>
                            <div class="twitter-image-wrapper">
                                <img <?php echo ( isset( $args['avatar_size'] ) ) ? 'width="' . $args['avatar_size'] . 'px" height="' . $args['avatar_size'] . 'px"' : 'width="48px" height="48px"'; ?> src="<?php echo esc_url( $t['image'] ); ?>" alt="<?php esc_html_e( 'X Avatar', 'basel' ); ?>">
                            </div>
						<?php endif ?>
