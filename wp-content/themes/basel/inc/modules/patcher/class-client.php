...
class Client extends Singleton {
	...
	public function get_count_pacthes_map() {
		...
		if ( 'admin.php' === $pagenow && isset( $_GET['page'] ) && ( 'basel_dashboard' === $_GET['page'] || 'xtemos_options' === $_GET['page'] ) ) { //phpcs:ignore
	...
	public function interface() {
		wp_enqueue_script( 'basel-patcher-scripts', BASEL_ASSETS . '/js/patcher.js', array(), basel_get_theme_info( 'Version' ), true );
				...
				<?php if ( $patches && $this->check_filesystem_api() ) : ?>
					<div class="xts-patch-button-wrapper <?php echo $all_patches_installed ? 'xts-applied' : ''; ?>">
						<a href="#" class="xts-btn xts-color-primary xts-patch-apply-all xts-i-check">
							<?php esc_html_e( 'Apply all', 'basel' ); ?>
						</a>
						<span class="xts-patch-label-applied xts-i-check">
							<?php esc_html_e( 'All applied', 'basel' ); ?>
						</span>
					</div>
				<?php endif; ?>
					...
					<div class="xts-patches-wrapper">
						...
						<div class="xts-patch-item xts-patch-title-wrapper">
							<div class="xts-patch-id">
								<?php esc_html_e( 'Patch ID', 'basel' ); ?>
							</div>
							<div class="xts-patch-description">
								<?php esc_html_e( 'Description', 'basel' ); ?>
							</div>
							<div class="xts-patch-date">
								<?php esc_html_e( 'Date', 'basel' ); ?>
							</div>
								...
								<div class="xts-patch-button-wrapper">
									...
									<?php else : ?>
										<a href="#" class="xts-btn xts-btn-primary xts-patch-apply" data-patches-map='<?php echo wp_json_encode( $patcher['files'] ); ?>' data-id="<?php echo esc_html( $patch_id ); ?>">
											<?php esc_html_e( 'Apply', 'basel' ); ?>
										</a>
										<span class="xts-patch-label-applied">
											<?php esc_html_e( 'Applied', 'basel' ); ?>
										</span>
									<?php endif; ?>
								</div>
							</div>
	...
	public function print_notices() {
		if ( ! $this->check_filesystem_api() ) {
			$this->notices['warning'] = esc_html__( 'Direct access to theme file is not allowed on your server. You need to download and replace the files manually.', 'basel' );
		}
	...
	private function print_notice( $message, $type = 'warning' ) {
		...
		<div class="xts-notice xts-<?php echo esc_attr( $type ); ?>">
			<?php echo wp_kses( $message, basel_get_allowed_html() ); ?>
		</div>
		<?php
	}
