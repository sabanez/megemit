...
class MKB_TemplateHelper {
	...
	public static function render_topics($settings = array()) {
		...
		if ($args['topics_title']) :
			$title_style = 'color: ' . $args['topics_title_color'] . ';font-size: ' . $settings['topics_title_size'] . ';';
			?>
		...
		if ( sizeof( $topics ) ):
			...
			?>
            <div align="center" style="padding-top: 30px;">Möchten Sie einen redaktionellen Beitrag/Case report in der Zeitschrift "Fokus Mikroimmuntherapie" bzw. in einem Newsletter veröffentlichen? 
            <br>Dann senden Sie uns bitte eine E-Mail an <a href="mailto:info@megemit.org" style="color: #0071b9;">info@megemit.org</a>.</div>
		</div>
		<?php
	}
	...
	public static function render_as_box($term, $settings = array()) {
		...
		?>
		<div class="kb-topic kb-topic--box-view">
			<?php
			if (!self::is_dynamic_topic($term)): ?>
