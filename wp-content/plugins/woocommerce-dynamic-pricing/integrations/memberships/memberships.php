...
function wc_dynamic_pricing_membership_applies_to_option( $module_name, $condition, $name, $condition_index ) {
	?>
    <option <?php selected( 'membership', $condition['args']['applies_to'] ); ?> value="membership"><?php esc_html_e( 'Membership Plan', 'woocommerce-dynamic-pricing' ); ?></option>
	<?php
}
...
function wc_dynamic_pricing_membership_applies_to_selector( $module_name, $condition, $name, $condition_index ) {
    ...
    <div class="membership" style="<?php echo esc_attr( $div_style ); ?>">
        <label for="pricing_rule_apply_to_<?php echo esc_attr( $name . '_' . $condition_index ); ?>_memberships"><?php esc_html_e('Membership Plans:', 'woocommerce-dynamic-pricing'); ?></label>

        <select style="width: 90%;" name="pricing_rules[<?php echo esc_attr( $name ); ?>][conditions][<?php echo esc_attr( $condition_index ); ?>][args][memberships][]" id="pricing_rule_apply_to_<?php echo esc_attr( $name . '_' . $condition_index ); ?>_memberships" class="multiselect wc-enhanced-select" multiple="multiple">
		    <?php foreach($all_plans as  $plan): ?>
                ...
                <option <?php selected($checked); ?> value="<?php echo esc_attr( $plan_id ); ?>"><?php echo esc_html( $plan->get_name() ); ?></option>
		    <?php endforeach; ?>
        </select>
    </div>
