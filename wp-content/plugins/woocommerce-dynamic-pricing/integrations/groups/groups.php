...
function wc_dynamic_pricing_groups_get_all_groups() {
   ...
   $results = wp_cache_get( $group_table, 'groups' );
    if ( false !== $results ) {
         return $results;
    }
    // phpcs:ignore WordPress.DB -- No other way to do this to get the groups.
    $results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM %i ORDER BY name", $group_table), ARRAY_A );
    wp_cache_set( $group_table, $results, 'groups' );
	return $results;
}
...
function wc_dynamic_pricing_groups_applies_to_option( $module_name, $condition, $name, $condition_index ) {
	?>
	<option <?php selected( 'groups', $condition['args']['applies_to'] ); ?> value="groups"><?php esc_html_e( 'Groups', 'woocommerce-dynamic-pricing' ); ?></option>
	<?php
}
...
function wc_dynamic_pricing_groups_applies_to_selector( $module_name, $condition, $name, $condition_index ) {
	...
	<div class="groups" style="<?php echo esc_attr( $div_style ); ?>">
		...
		<?php foreach ( $chunks as $chunk ) : ?>
			<ul class="list-column">
				<?php foreach ( $chunk as $group ) : ?>
					...
					<?php $group_checked = isset( $condition['args']['groups'] ) && is_array( $condition['args']['groups'] ) && in_array( $group_id, $condition['args']['groups'] ); ?>
					<li>
						<label for="<?php echo esc_attr( $name ); ?>_group_<?php echo esc_attr( $group_id ); ?>" class="selectit">
							<input <?php checked($group_checked); ?> type="checkbox" id="<?php echo esc_attr( $name ); ?>_group_<?php echo esc_attr( $group_id ); ?>" name="pricing_rules[<?php echo esc_attr( $name ); ?>][conditions][<?php echo esc_attr( $condition_index ); ?>][args][groups][]" value="<?php echo esc_attr( $group_id ); ?>" /><?php echo esc_html( $group['name'] ); ?>
						</label>
					</li>
				<?php endforeach; ?>
			</ul>
