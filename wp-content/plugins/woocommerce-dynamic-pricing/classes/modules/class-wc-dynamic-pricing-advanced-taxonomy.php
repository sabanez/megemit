...
class WC_Dynamic_Pricing_Advanced_Taxonomy extends WC_Dynamic_Pricing_Advanced_Base {
	...
	public function adjust_cart( $temp_cart ) {
		...
		if ( $this->adjustment_sets && count( $this->adjustment_sets ) ) {
			...
			$valid_sets = array_filter( $valid_sets, function( $set ) {
			...
			foreach ( $valid_sets as $set_id => $set ) {
				...
				foreach ( $temp_cart as $cart_item_key => &$cart_item ) {
					...
					// Make sure terms is not a WP_Error and is an array.
	...
	public function get_adjustment_sets() {
		return apply_filters( 'wc_dynamic_pricing_get_adjustment_sets', ( empty( $this->adjustment_sets ) ? false : $this->adjustment_sets ), 'advanced_taxonomy', $this->taxonomy );
	}
