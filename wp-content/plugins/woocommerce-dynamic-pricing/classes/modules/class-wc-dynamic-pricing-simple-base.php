...
abstract class WC_Dynamic_Pricing_Simple_Base extends WC_Dynamic_Pricing_Module_Base {
	...
	public array $available_rulesets = array();
	...
	abstract public function is_applied_to_product( $product );
	 ...
	 * @return bool|float
	 */
	abstract public function get_discounted_price_for_shop( $product, $working_price );
		...
		if ( isset( WC()->cart->cart_contents[ $cart_item_key ]['discounts'] ) ) {
			if ( in_array( $this->module_id, WC()->cart->cart_contents[ $cart_item_key ]['discounts']['by'] ) ) {
				...
				array_intersect(
					...
					WC()->cart->cart_contents[ $cart_item_key ]['discounts']['by']
				)
			) > 0 ) {
				$cumulative = true;
			}
		} else {
			$cumulative = $default_value;
		}
