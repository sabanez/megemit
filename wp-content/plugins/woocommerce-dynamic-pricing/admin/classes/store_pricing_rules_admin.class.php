...
class woocommerce_store_pricing_rules_admin {
    ...
    public function __construct() {
        ...
        if ( is_admin() ) {
            ...
            add_action( 'admin_menu', [ $this, 'on_admin_menu' ], 99 );
    ...
    public function enqueue( $hook ) {
        ...
        if ( $hook == 'woocommerce_page_wc_dynamic_pricing' || ( $post && $post->post_type == 'product' ) ) {
            if ( floatval( WC()->version ) >= 2.0 ) {
                ...
                wp_enqueue_script(
                        'woocommerce-pricing-admin',
                        WC_Dynamic_Pricing::plugin_url() . '/assets/admin/admin.js',
                        [
                                'jquery',
                                'jquery-ui-core',
                                'jquery-ui-sortable',
                ...
                wp_enqueue_script( 'woocommerce-pricing-admin', WC_Dynamic_Pricing::plugin_url() . '/assets/legacy/admin/admin.js', [ 'jquery' ] );
            }
    ...
    public function on_admin_page() {
        // phpcs:disable WordPress.Security.NonceVerification.Recommended -- Admin page navigation, no state changes
        $current_tab  = isset( $_GET['tab'] ) ? sanitize_text_field( wp_unslash( $_GET['tab'] ) ) : 'order totals';
        $current_view = isset( $_GET['view'] ) ? sanitize_text_field( wp_unslash( $_GET['view'] ) ) : 0;
        ...
        <div class="wrap woocommerce">
            ...
            <h2 class="nav-tab-wrapper woo-nav-tab-wrapper">
                <?php
                $tabs = apply_filters(
                        'woocommerce_dynamic_pricing_tabs',
                        [
                                'order totals' => [
                                        'tabs' => [
                                                [
                ...
                foreach ( $tabs as $name => $value ) :
                    if ( $current_tab == $name ) :
                        ?>
                        <a href="<?php echo esc_attr( admin_url( 'admin.php?page=wc_dynamic_pricing&tab=' . $name ) ); ?>"
                           class="nav-tab active-nav-tab nav-tab-active">
                            <?php
                            if ( isset( $value['tab_title'] ) ) {
                                echo esc_html( $value['tab_title'] );
                            } else {
            ...
            <?php if ( isset( $tabs[ $current_tab ]['tabs'][ $current_view ] ) ) : ?>
                <?php if ( ! isset( $tabs[ $current_tab ]['tabs'][ $current_view ]['hide_title'] ) || $tabs[ $current_tab ]['tabs'][ $current_view ]['hide_title'] != true ) : ?>
                    <div class="tab_top">
                        <h3 class="has-help"><?php echo esc_html( $tabs[ $current_tab ]['tabs'][ $current_view ]['title'] ); ?></h3>
                        <?php if ( $tabs[ $current_tab ]['tabs'][ $current_view ]['description'] ) : ?>
                            <p class="help"><?php echo esc_html( $tabs[ $current_tab ]['tabs'][ $current_view ]['description'] ); ?></p>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
                <?php
    ...
    public function register_settings() {
        ...
        register_setting(
                '_a_taxonomy_product_brand_pricing_rules',
                '_a_taxonomy_product_brand_pricing_rules',
                [
                        $this,
        ...
        register_setting(
                '_s_taxonomy_product_brand_pricing_rules',
                '_s_taxonomy_product_brand_pricing_rules',
                [
                        $this,
                        'on_store_settings_validation',
        ...
        foreach ( $this->taxonomy_admins as $taxonomy => $handler ) {
            if ( $taxonomy != 'product_brand' ) {
                ...
                register_setting(
                        '_a_taxonomy_' . $taxonomy . '_pricing_rules',
                        '_a_taxonomy_' . $taxonomy . '_pricing_rules',
                        [
                                $this,
                                'on_store_settings_validation',
    ...
    public function on_store_settings_validation( $data ) {
        $rules             = [];
        $pricing_rule_sets = $this->get_posted_pricing_rules();
        ...
        if ( $pricing_rule_sets ) {
            foreach ( $pricing_rule_sets as $key => $rule_set ) {
                ...
                foreach ( $rule_set['rules'] as &$rule ) {
                    ...
                    $rule['from']   = sanitize_text_field( $rule['from'] );
                    $rule['to']     = sanitize_text_field( $rule['to'] );
                    $rule['amount'] = sanitize_text_field( $rule['amount'] );
    ...
    public function clear_transients() {
        ...
        // phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery -- Bulk transient cleanup, more efficient than individual WordPress calls
        $wpdb->query(
                $wpdb->prepare(
                        "DELETE FROM $wpdb->options WHERE option_name LIKE %s OR option_name LIKE %s",
                        $wpdb->esc_like( '_transient_wc_var_prices' ) . '%',
                        $wpdb->esc_like( '_transient_timeout_wc_var_prices' ) . '%'
                )
        );
    ...
    private function get_posted_pricing_rules() {
        $rules = false;
        // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Not needed, WordPress Settings API handles nonce verification
        if ( isset( $_POST['pricing_rules'] ) ) {
            // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Settings API handles nonce, data sanitized per-field where needed
            $rules = wp_unslash( $_POST['pricing_rules'] );
        }
