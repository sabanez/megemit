...
final class JobTableMigration
{
    public const TABLE = 'borlabs_cookie_jobs';

    private JobInstall $jobInstall;

    private JobUpgrade $jobUpgrade;
    ...
    public function run(string $prefix = ''): AuditDto
    {
        ...
        if (Database::tableExists($prefix . self::TABLE) === false) {
            $createStatus = $this->jobInstall->createTable($prefix);
        ...
        $upgradeStatus = $this->jobUpgrade->upgrade($prefix);
