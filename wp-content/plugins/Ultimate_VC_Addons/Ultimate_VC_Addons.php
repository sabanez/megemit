...
function on_ultimate_vc_addons_activate() {
	...
	// Add redirect to dashboard on activation
	add_option('uavc_activation_redirect', true);
}
...
if ( ! defined( 'ULTIMATE_VERSION' ) ) {
	define( 'ULTIMATE_VERSION', '3.21.3' );
}
...
if ( ! class_exists( 'Ultimate_VC_Addons' ) ) {
	...
	class Ultimate_VC_Addons {
		...
		public function __construct() {
			...
			self::$uavc_editor_enable = is_admin() || ( isset( $_GET['vc_action'] ) && 'vc_inline' == $_GET['vc_action'] ) || ( isset( $_GET['vc_editable'] ) && isset( $_GET['_vcnonce'] ) && wp_verify_nonce( sanitize_text_field( $_GET['_vcnonce'] ), 'vc-nonce-vc-admin-nonce' ) && sanitize_text_field( $_GET['vc_editable'] ) );
			...
			add_action( 'wp_ajax_uavc_recommended_plugin_activate', [ $this, 'uavc_activate_addon' ] );
			add_action( 'wp_ajax_uavc_recommended_plugin_install', [ $this, 'uavc_plugin_install' ] );
			add_action( 'wp_ajax_uavc_recommended_theme_install', [ $this, 'uavc_theme_install' ] );
			...
			require_once UAVC_DIR . 'inc/settings/uavc-settings-api.php';
			require_once UAVC_DIR . 'inc/rest-api.php';

			// Load the NPS Survey library.
			if ( ! class_exists( 'Uavc_Nps_Survey' ) ) {
				require_once UAVC_DIR . 'lib/class-uavc-nps-survey.php';
			}
			...
			$bsf_analytics = BSF_Analytics_Loader::get_instance();
						...
						'deactivation_survey' => array(
							array(
								'id'                => 'deactivation-survey-ultimate-addons-for-wpbakery-page-builder', // 'deactivation-survey-<your-plugin-slug>'
								'popup_logo'        => UAVC_URL . 'assets/images/UAVC_logo.svg',
								'plugin_slug'       => 'ultimate-addons-for-wpbakery-page-builder', // <your-plugin-slug>
								'plugin_version'    => ULTIMATE_VERSION,
								'popup_title'       => __( 'Quick Feedback' ),
								'support_url'       => 'https://ultimate.brainstormforce.com/support/',
								'popup_description' => __( 'If you have a moment, please share why you are deactivating Ultimate Addons for WPBakery Page Builder:'),
                        ...
                        // Remove default analytics setting and notice from WordPress General page.
                        remove_action( 'admin_init', array( $bsf_analytics, 'register_usage_tracking_setting' ) );
                        remove_action( 'admin_init', array( $bsf_analytics, 'option_notice' ) );

                        // Migrate old usage tracking option if present.
                        $this->uavc_maybe_migrate_usage_tracking();
                }
		 ...
		 * @return void
		 */
		public function uavc_activate_addon() {
			...
			if ( isset( $_POST['plugin'] ) ) {
				...
				if ( 'plugin' === $type ) {
					...
					if ( ! current_user_can( 'activate_plugins' ) ) {
						wp_send_json_error( esc_html__( 'Plugin activation is disabled for you on this site.', 'ultimate_vc' ) );
					}
					...
					if ( ! is_wp_error( $activate ) ) {
						...
						do_action( 'uavc_plugin_activated', $plugin );

						wp_send_json_success( esc_html__( 'Plugin Activated.', 'ultimate_vc' ) );
					}
				}
				...
				if ( 'theme' === $type ) {
					...
					if ( isset( $_POST['slug'] ) ) {
						...
						if ( ! ( current_user_can( 'switch_themes' ) ) ) {
							wp_send_json_error( esc_html__( 'Theme activation is disabled for you on this site.', 'ultimate_vc' ) );
						}
						...
						if ( ! is_wp_error( $activate ) ) {
							...
							do_action( 'uavc_theme_activated', $plugin );

							wp_send_json_success( esc_html__( 'Theme Activated.', 'ultimate_vc' ) );
						}
					}
				}
			}
			...
			if ( 'plugin' === $type ) {
				wp_send_json_error( esc_html__( 'Could not activate plugin. Please activate from the Plugins page.', 'ultimate_vc' ) );
			} elseif ( 'theme' === $type ) {
				wp_send_json_error( esc_html__( 'Could not activate theme. Please activate from the Themes page.', 'ultimate_vc' ) );
			}
		}
		 ...
		 * @since 2.2.0
		 */
		public function uavc_plugin_install() {

			check_ajax_referer( 'updates', '_ajax_nonce' );

			if ( ! current_user_can( 'install_plugins' ) ) {
				wp_send_json_error( array( 'message' => __( 'You do not have permission to install plugins.', 'ultimate_vc' ) ) );
			}
			...
			if ( empty( $plugin_slug ) ) {
				wp_send_json_error( array( 'message' => __( 'Plugin slug is missing.', 'ultimate_vc' ) ) );
			}
				...
				function () use ( $plugin_slug ) {
					...
					foreach ( $all_plugins as $plugin_file => $_ ) {
						if ( class_exists( 'BSF_UTM_Analytics' ) && is_callable( 'BSF_UTM_Analytics::update_referer' ) && strpos( $plugin_file, $plugin_slug . '/' ) === 0 ) {
							...
							BSF_UTM_Analytics::update_referer( 'Ultimate_VC_Addons', $plugin_slug );
							return;
						}
					}
				}
			...
			if ( function_exists( 'wp_ajax_install_plugin' ) ) {
				...
				wp_send_json_error( array( 'message' => __( 'Plugin installation function not found.', 'ultimate_vc' ) ) );
			}
		}
		 ...
		 * @since 2.2.0
		 */
		public function uavc_theme_install() {

			check_ajax_referer( 'updates', '_ajax_nonce' );

			if ( ! current_user_can( 'install_themes' ) ) {
				wp_send_json_error( array( 'message' => __( 'You do not have permission to install themes.', 'ultimate_vc' ) ) );
			}
			...
			if ( empty( $theme_slug ) ) {
				wp_send_json_error( array( 'message' => __( 'Theme slug is missing.', 'ultimate_vc' ) ) );
			}
				...
				function () use ( $theme_slug ) {
					...
					foreach ( $all_themes as $theme_file => $_ ) {
						if ( class_exists( 'BSF_UTM_Analytics' ) && is_callable( 'BSF_UTM_Analytics::update_referer' ) && strpos( $theme_file, $theme_slug . '/' ) === 0 ) {
							...
							BSF_UTM_Analytics::update_referer( 'Ultimate_VC_Addons', $theme_slug );
							return;
						}
					}
				}
			...
			if ( function_exists( 'wp_ajax_install_theme' ) ) {
				...
				wp_send_json_error( array( 'message' => __( 'Theme installation function not found.', 'ultimate_vc' ) ) );
			}
		}
		 ...
		 */
		public function ultimate_vc_addons_bsf_registration_page_url() {   // PHPCS:Ignore WordPressVIPMinimum.Hooks.AlwaysReturnInFilter.MissingReturnStatement
			if ( is_multisite() ) {
		...
		public static function show_nps_notice() {
			if ( class_exists( 'Nps_Survey' ) ) {
				\Nps_Survey::show_nps_notice(
					'nps-survey-ultimate_vc',
					array(
						'show_if'          => true, // Add your display conditions.
						'dismiss_timespan' => 2 * WEEK_IN_SECONDS,
						'display_after'    => 2 * WEEK_IN_SECONDS,
						'plugin_slug'      => 'ultimate_vc',
						'show_on_screens'  => array( 'toplevel_page_uavc', 'ultimate_page_uavc', 'toplevel_page_about-ultimate', 'ultimate_page_ultimate-dashboard', 'ultimate_page_ultimate-smoothscroll', 'ultimate_page_ultimate-scripts-and-styles', 'ultimate_page_bsf-font-icon-manager', 'ultimate_page_bsf-google-font-manager', 'ultimate_page_bsf-google-maps'  ),
						'message'          => array(
							// Step 1 i.e rating input.
							'logo'                  => esc_url( UAVC_URL . 'admin/img/nps_uavc.svg' ),
							'plugin_name'           => __( 'Ultimate Addons for WPBakery', 'ultimate_vc' ),
							'nps_rating_message'    => __( 'How likely are you to recommend Ultimate Addons for WPBakery to your friends or colleagues?', 'ultimate_vc' ),
							// Step 2A i.e. positive.
							'feedback_title' => __( 'Thanks a lot for your feedback! 😍', 'ultimate_vc' ),
							'feedback_content' => __( 'Thanks for using Ultimate Addons! Got feedback or suggestions to make it even better? We’d love to hear from you.', 'ultimate_vc' ),
							'plugin_rating_link'    => esc_url( 'https://codecanyon.net/item/ultimate-addons-for-visual-composer/reviews/6892199' ),
							// Step 2B i.e. negative.
							'plugin_rating_title'   => __( 'Thank you for your feedback', 'ultimate_vc' ),
							'plugin_rating_content' => __( 'We value your input. How can we improve your experience?', 'ultimate_vc' ),
						),
					)
				);
			}
		 ...
		 */
		public function uavc_bsf_registration_page_url() { // PHPCS:Ignore WordPressVIPMinimum.Hooks.AlwaysReturnInFilter.MissingReturnStatement
			if ( is_multisite() ) {
				return network_admin_url( 'plugins.php?bsf-inline-license-form=6892199' );
			} else {
		...
		public function uvc_plugin_activate() {
			...
			if ( $memory_limit - $peak_memory <= 14436352 && ! defined( 'WP_CLI' ) ) {
				...
				$msg .= '<br/><br/>' . esc_html__( 'Please check ', 'ultimate_vc' ) . '<a target="_blank" href="https://ultimate.brainstormforce.com/docs/increasing-memory-limit/">' . esc_html__( 'this article', 'ultimate_vc' ) . '</a> ';
		...
		public function ultimate_plugins_page_link( $links ) {
                       ...
                       $settins_link  = '<a href="' . admin_url( 'admin.php?page=uavc&tab=1#settings' ) . '" target="_blank">' . esc_html__( 'Settings', 'ultimate_vc' ) . '</a>';
		...
		public function aio_move_templates() {
			...
			// PHPCS:Disable WordPressVIPMinimum.Functions.RestrictedFunctions.chmod_chmod
			@chmod( $this->vc_dest_dir, 0777 ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
			// PHPCS:Enable WordPressVIPMinimum.Functions.RestrictedFunctions.chmod_chmod
		...
		public function toggle_updater() {
			...
			if ( ! $ultimate_modules && ! is_array( $ultimate_modules ) ) {
				// Create associative array with same key and value
				$assoc_modules = array_combine( $modules, $modules );
				update_option( 'ultimate_modules', $assoc_modules );
			}
			...
			if (get_option('uavc_activation_redirect')) {
				delete_option('uavc_activation_redirect');
				if (!is_network_admin() && !isset($_GET['activate-multi'])) {
					wp_redirect(admin_url('admin.php?page=uavc#dashboard'));
					exit;
				}
			}
                ...
                /**
                 * Migrates usage tracking option from 'bsf_usage_optin' to 'uavc_usage_optin'.
                 *
                 * @since 3.20.4
                 * @return void
                 */
                public function uavc_maybe_migrate_usage_tracking() {
                        $old_tracking = get_option( 'bsf_usage_optin', false );
                        $new_tracking = get_option( 'uavc_usage_optin', false );

                        if ( 'yes' === $old_tracking && false === $new_tracking ) {
                                update_option( 'uavc_usage_optin', 'yes' );
                                $time = get_option( 'bsf_usage_installed_time' );
                                update_option( 'uavc_usage_installed_time', $time );
                        }
                }
