	...
	class Ultimate_VC_Addons_Icon_Manager {
		...
		public function __construct() {
			...
			if ( isset( $_GET['delete-bsf-fonts'] ) ) {// @codingStandardsIgnoreLine.
				if ( current_user_can( 'manage_options' ) && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( wp_unslash( $_GET['_wpnonce'] ), 'delete-bsf-fonts' ) ) {
		...
		public function add_zipped_font() {
			...
			$attachment = isset( $_POST['values'] ) ? array_map( 'sanitize_text_field', $_POST['values'] ) : array();
		...
		public function remove_zipped_font() {
			...
			// get the file path of the zip file.
			$font   = isset( $_POST['del_font'] ) ? sanitize_text_field( $_POST['del_font'] ) : '';
		...
		public function create_config() {
			...
			if ( '' !== $json && '' !== $response ) {
				...
				// Sanitize font name to prevent directory traversal and code injection.
				$this->font_name = preg_replace( '/[^a-zA-Z0-9_\-]/', '', $this->font_name );
		...
		public function write_config() {
			...
			if ( $handle ) {
				...
				foreach ( $this->json_config[ $this->font_name ] as $icon => $info ) {
					if ( ! empty( $info ) ) {
						// Sanitize the tags.
						$sanitized_tags = array_map( 'esc_html', explode( ',', $info['tags'] ) );
						$tags           = implode( ',', $sanitized_tags );

						// Write to file safely.
						$delimiter = "'";
						fwrite( $handle, "\r\n" . '$icons[\'' . $this->font_name . '\'][' . $delimiter . $icon . $delimiter . '] = array("class"=>' . $delimiter . $info['class'] . $delimiter . ',"tags"=>' . $delimiter . $tags . $delimiter . ',"unicode"=>' . $delimiter . $info['unicode'] . $delimiter . ');' );// @codingStandardsIgnoreLine.
					} else {
						$this->delete_folder( $this->paths['tempdir'] );
			...
			foreach ( glob( $folder . '*' ) as $file ) {
				...
				if ( strpos( $path_parts['filename'], '.dev' ) === false && in_array( $path_parts['extension'], $extensions ) ) {
					if ( $path_parts['filename'] !== $this->font_name ) {
						rename( $file, trailingslashit( $path_parts['dirname'] ) . $this->font_name . '.' . $path_parts['extension'] ); // PHPCS:Ignore WordPressVIPMinimum.Functions.RestrictedFunctions.file_ops_rename
					}
				}
			}
		}
		...
		public function rename_folder() {
			...
			if ( rename( $this->paths['tempdir'], $new_name ) ) { // PHPCS:Ignore WordPressVIPMinimum.Functions.RestrictedFunctions.file_ops_rename
				return true;
			} else {
				$this->delete_folder( $this->paths['tempdir'] );
		...
		public function delete_folder( $new_name ) {
			...
			if ( is_dir( $new_name ) ) {
				...
				foreach ( $objects as $object ) {
					if ( '.' != $object && '..' != $object ) {
						unlink( $new_name . '/' . $object ); // PHPCS:Ignore WordPressVIPMinimum.Functions.RestrictedFunctions.file_ops_unlink
					}
				}
				reset( $objects );
				rmdir( $new_name ); // PHPCS:Ignore WordPressVIPMinimum.Functions.RestrictedFunctions.directory_rmdir
			} else {
		...
		public function AIO_move_fonts() { // @codingStandardsIgnoreLine.
			...
			@chmod( $this->vc_fonts, 0755 );// @codingStandardsIgnoreLine.
	...
	*/
	if ( ! function_exists( 'smile_backend_create_folder' ) ) {
		...
		function smile_backend_create_folder( &$folder, $addindex = true ) {
			...
			@chmod( $folder, 0755 );// @codingStandardsIgnoreLine.
			if ( false == $addindex ) {
				return $created;
			}
