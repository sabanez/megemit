...
if ( ! class_exists( 'Ultimate_VC_Addons_IHover' ) ) {
	...
	class Ultimate_VC_Addons_IHover {
		...
		public function ult_ihover_item_callback( $atts, $content = null ) {
			...
			if ( ! in_array( $ult_ihover_settings['heading_tag'], $valid_tags ) ) {
				$ult_ihover_settings['heading_tag'] = 'h3';
			}
		...
		public function common_structure( $desc_responsive, $title_responsive, $heading_block, $title_style, $ult_ihover_settings, $spacer_style, $spacer_line_style, $description_block, $desc_style, $content ) {
			...
			$item_output .= '				<' . esc_attr( $ult_ihover_settings['heading_tag'] ) . ' class="ult-ih-heading ult-responsive" style="' . esc_attr( $title_style ) . '" ' . $title_responsive . '>' . esc_html( $ult_ihover_settings['title'] ) . '</' . esc_attr( $ult_ihover_settings['heading_tag'] ) . '>';
			$item_output .= '			</div>';
