...
if ( ! class_exists( 'Ultimate_VC_Addons_Info_List' ) ) {
	...
	class Ultimate_VC_Addons_Info_List {
		...
		public function info_list_item( $atts, $content = null ) {
			...
			if ( ! in_array( $ult_info_list_item['heading_tag'], $valid_tags ) ) {
				$ult_info_list_item['heading_tag'] = 'h3';
			}
			...
			if ( '' != $ult_info_list_item['list_title'] ) {
				...
				if ( $is_link && 'title' == $ult_info_list_item['info_list_link_apply'] ) {
					$output .= '<a ' . Ultimate_VC_Addons::uavc_link_init( $url, $target, $link_title, $rel ) . '>' . wp_kses_post( $ult_info_list_item['list_title'] ) . '</a>';
				} else {
					$output .= wp_kses_post( $ult_info_list_item['list_title'] );
				}
		...
		public function front_info_list( $atts, $content = null ) {
			...
			$output  = '<div class="smile_icon_list_wrap ' . esc_attr( $ult_info_list_front['el_class'] ) . '' . esc_attr( isset( $css_info_list ) ? $css_info_list : '' ) . '">';
		...
		public function front_info_list_item( $atts, $content = null ) {
			...
			// Ensuring the custom icon is set and getting the custom icon URL with styles.
			if ( 'custom' == $ult_info_list_front_item['icon_type'] ) {
			...
			$output .= '<' . esc_attr( isset( $heading_tag ) ? $heading_tag : 'h3' ) . '>' . wp_kses_post( $ult_info_list_front_item['list_title'] ) . '</' . esc_attr( isset( $heading_tag ) ? $heading_tag : '' ) . '>';
			$output .= wpb_js_remove_wpautop( $content, true );
			$output .= '</div>';
			$output .= '<div class="icon_list_connector frontend_editor_list_connector" ' . $connector_trans . ' style="' . esc_attr( $this->connect_color_style ) . ';"></div>';
			$output .= '</li>';
			return $output;
		}
		/**
