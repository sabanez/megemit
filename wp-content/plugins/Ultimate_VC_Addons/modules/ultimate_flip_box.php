...
if ( ! class_exists( 'Ultimate_VC_Addons_Flip_Box' ) ) {
	...
	class Ultimate_VC_Addons_Flip_Box {
		...
		public function block_shortcode( $atts ) {
			...
			// Front Heading tag.
			if ( ! in_array( $ult_flip_settings['front_heading_tag'], $valid_tags ) ) {
				$ult_flip_settings['front_heading_tag'] = 'h3';
			}

			// Back Heading tag.
			if ( ! in_array( $ult_flip_settings['back_heading_tag'], $valid_tags ) ) {
				$ult_flip_settings['back_heading_tag'] = 'h3';
			}
			...
			$output     .= '<div class="flip-box-wrap" tabindex="0" style="' . esc_attr( $flip_design_style ) . '">';
