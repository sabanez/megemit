		...
		public function info_circle( $atts, $content = null ) {
			...
			if ( stripos( $browser_info['name'], 'safari' ) == true && stripos( $browser_info['version'], '11.' ) == true ) {
				if ( 'none' != $ult_info_circle['eg_br_style'] && '' != $ult_info_circle['eg_br_width'] && '' != $ult_info_circle['eg_border_color'] ) {
					$style .= 'box-shadow: 0 0 0 ' . esc_attr( $ult_info_circle['eg_br_width'] ) . 'px ' . esc_attr( $ult_info_circle['eg_border_color'] ) . ';';
				}
			} else {
				if ( 'none' != $ult_info_circle['eg_br_style'] && '' != $ult_info_circle['eg_br_width'] && '' != $ult_info_circle['eg_border_color'] ) {
					$style .= 'border:' . esc_attr( $ult_info_circle['eg_br_width'] ) . 'px ' . esc_attr( $ult_info_circle['eg_br_style'] ) . ' ' . esc_attr( $ult_info_circle['eg_border_color'] ) . ';';
				}
			}
			if ( stripos( $browser_info['name'], 'safari' ) == true && stripos( $browser_info['version'], '11.' ) == true ) {
				if ( 'none' != $ult_info_circle['cn_br_style'] && '' != $ult_info_circle['cn_br_width'] && '' != $ult_info_circle['cn_border_color'] ) {
					$style1 .= 'box-shadow: 0 0 0 ' . esc_attr( $ult_info_circle['cn_br_width'] ) . 'px ' . esc_attr( $ult_info_circle['cn_border_color'] ) . ';';
				}
			} else {
				if ( 'none' != $ult_info_circle['cn_br_style'] && '' != $ult_info_circle['cn_br_width'] && '' != $ult_info_circle['cn_border_color'] ) {
					$style1 .= 'border:' . esc_attr( $ult_info_circle['cn_br_width'] ) . 'px ' . esc_attr( $ult_info_circle['cn_br_style'] ) . ' ' . esc_attr( $ult_info_circle['cn_border_color'] ) . ';';
				}
			}
			$style1 .= 'background-color:' . esc_attr( $ult_info_circle['content_bg'] ) . ';color:' . esc_attr( $ult_info_circle['content_color'] ) . ';';
			$style1 .= 'width:' . esc_attr( $ult_info_circle['eg_padding'] ) . '%;height:' . esc_attr( $ult_info_circle['eg_padding'] ) . '%;margin:' . esc_attr( ( ( 100 - $ult_info_circle['eg_padding'] ) / 2 ) ) . '%;';
			...
			if ( ! empty( $href['url'] ) ) {
				...
				$output    .= '<div class="info-circle-def"><div  class="info-circle-sub-def">
							<a class="info-circle-href" ' . Ultimate_VC_Addons::uavc_link_init( $url, $target, $link_title, $rel ) . ' style="color:inherit;">' . $icon_html . '</a>
								<div class="responsive-font-class ult-responsive" ' . $info_circle_desc_data_list . '><h3 ' . $uavc_info_circle_data_list . ' class="info-circle-heading ult-responsive new-cust-responsive-class" style="' . esc_attr( $uavc_title_style_inline ) . '">' . wp_kses_post( $ult_info_circle_settings['info_title'] ) . '</h3>
								<div ' . $info_circle_desc_data_list . ' class="info-circle-text " style="' . esc_attr( $uavc_desc_style_inline ) . '">' . do_shortcode( $content ) . '</div>
							</div></div></div></div>';
			} else {
				$output .= '<div class="info-circle-def"><div  class="info-circle-sub-def">' . $icon_html . '<div class="responsive-font-class ult-responsive" ' . $info_circle_desc_data_list . '><h3 ' . $uavc_info_circle_data_list . ' class="info-circle-heading ult-responsive new-cust-responsive-class" style="' . esc_attr( $uavc_title_style_inline ) . '">' . wp_kses_post( $ult_info_circle_settings['info_title'] ) . '</h3><div ' . $info_circle_desc_data_list . ' class="info-circle-text " style="' . esc_attr( $uavc_desc_style_inline ) . '">' . do_shortcode( $content ) . '</div></div></div></div></div>';
			}
			return $output;
		}
		/**
