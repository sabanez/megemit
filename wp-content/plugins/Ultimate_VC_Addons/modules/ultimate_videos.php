	...
	class Ultimate_VC_Addons_Videos {
		...
		public function get_video_thumb( $setting ) {
			...
			if ( 'custom' == $setting['thumbnail'] ) {
				...
				if ( 'uv_iframe' == $setting['video_type'] ) {
					...
					$vimeo_response = wp_remote_get( "https://vimeo.com/api/v2/video/{$id}.json" );
					if ( is_wp_error( $vimeo_response ) || 200 !== wp_remote_retrieve_response_code( $vimeo_response ) ) {
						return '';
					}
					$vimeo = json_decode( wp_remote_retrieve_body( $vimeo_response ), true );
					if ( ! is_array( $vimeo ) || empty( $vimeo[0]['thumbnail_large'] ) ) {
						return '';
					}
		...
		public function get_header_wrap( $setting ) {
			...
			if ( isset( $id ) && '' != $id ) {
				...
				$vimeo = json_decode( wp_remote_retrieve_body( $vimeo_response ), true );
				if ( ! is_array( $vimeo ) || empty( $vimeo[0] ) ) {
					return '';
				}
				...
				'on' == $setting['vimeo_intro_byline']
					...
					<a href="' . esc_url( $vimeo[0]['user_url'] ) . '"><img src="' . esc_url( $vimeo[0]['user_portrait_huge'] ) . '"></a></div>';
					}
					...
					'on' == $setting['vimeo_intro_byline']
						...
						if ( 'on' == $setting['vimeo_intro_title'] ) {
							...
							<a href="' . esc_url( $setting['vimeo_video_url'] ) . '">' . esc_html( $vimeo[0]['title'] ) . '</a>
						</div>';
						}
						if ( 'on' == $setting['vimeo_intro_byline'] ) {
							$html .= '<div class="ultv-vimeo-byline">
						' . esc_html__( 'from ', 'ultimate_vc' ) . '<a href="' . esc_url( $setting['vimeo_video_url'] ) . '"> ' . esc_html( $vimeo[0]['user_name'] ) . '</a>
					</div>';
						}
						$html .= '</div>';
					}
		...
		public function get_video_embed( $setting, $uid ) {
			...
			$style                   = '';
			$hover_color             = '';
			// Ensure $yt_txt and $yt_txt_back are defined
			$yt_txt      = '';
			$yt_txt_back = '';
			...
			$output          = '<div class="ultv-video ultv-aspect-ratio-' . esc_attr( $setting['aspect_ratio'] ) . ' ultv-subscribe-responsive-' . esc_attr( $setting['subscribe_bar_responsive'] ) . '" data-videotype="' . esc_attr( $setting['video_type'] ) . '">
						<div class="ultv-video__outer-wrap" data-autoplay="' . esc_attr( $autoplay ) . '" data-device="' . esc_attr( $device ) . '" ' . $hover_color . ' data-iconbg="' . esc_attr( $setting['icon_color'] ) . '" data-overcolor="' . esc_attr( $setting['overlay_color'] ) . '" data-defaultbg="' . esc_attr( $setting['default_color'] ) . '" data-defaultplay="' . esc_attr( $setting['play_source'] ) . '">';
					$output .= $this->get_header_wrap( $setting );
					$output .= '<div class="ultv-video__play" data-src="' . esc_url( $src ) . '">
								<img class="ultv-video__thumb" src="' . esc_url( $this->get_video_thumb( $setting ) ) . '"/>
								<div class="ultv-video__play-icon ' . esc_attr( ( 'icon' == $setting['play_source'] ) ? $setting['play_icon'] : '' ) . ' ultv-animation-' . esc_attr( $setting['hover_animation'] ) . '" style="' . esc_attr( $setting['play_size'] ) . '">';
			if ( 'image' == $setting['play_source'] ) {
				$output .= '<img src="' . esc_url( $html ) . '"/>';
			}
			if ( 'defaulticon' == $setting['play_source'] ) {
				$output .= $html;
			}
			...
			if ( 'uv_iframe' == $setting['video_type'] && 'on' == $setting['enable_sub_bar'] ) {
				...
				<div class="ultv-subscribe-bar-prefix" style="' . esc_attr( $main_video_style_inline ) . '">' . esc_html( $youtube_text ) . '</div>
					...
					array(
						...
						'params'      => array(
							...
							array(
								...
								'description' => __( 'Click' ) . " <a href='https://ultimate.brainstormforce.com/docs/how-to-find-youtube-channel-name-and-channel-id/' target='_blank' rel='noopener'>" . __( 'here', 'ultimate_vc' ) . '</a>' . __( ' to find your YouTube Channel Name.', 'ultimate_vc' ),
								'dependency'  => array(
							...
							array(
								...
								'description' => __( 'Click' ) . " <a href='https://ultimate.brainstormforce.com/docs/how-to-find-youtube-channel-name-and-channel-id/' target='_blank' rel='noopener'>" . __( 'here', 'ultimate_vc' ) . '</a>' . __( ' to find your YouTube Channel ID.', 'ultimate_vc' ),
								'dependency'  => array(
									'element' => 'chanel_id_name',
