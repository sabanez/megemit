...
if ( ! class_exists( 'Ultimate_VC_Addons_Headings' ) ) {
	...
	class Ultimate_VC_Addons_Headings {
		...
		public function ultimate_headings_shortcode( $atts, $content = null ) {
			...
			$wrapper_class                    = esc_attr( $ult_heading_settings['spacer'] );

			// Ensure heading_tag is a valid HTML tag.
			$valid_tags = array( 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'div', 'p', 'span' );

			// Dual-color Heading tag.
			if ( ( '' == sanitize_text_field( $ult_heading_settings['heading_tag'] ) ) || ( ! in_array( $ult_heading_settings['heading_tag'], $valid_tags ) ) ) {
			...
			if ( '' != $ult_heading_settings['main_heading_color'] ) {
				$main_heading_style_inline .= 'color:' . esc_attr( $ult_heading_settings['main_heading_color'] ) . ';';
			}
			// attach margins for main heading.
			if ( '' != $ult_heading_settings['main_heading_margin'] ) {
				$main_heading_style_inline .= esc_attr( $ult_heading_settings['main_heading_margin'] );
			}
			...
			if ( '' != $ult_heading_settings['sub_heading_color'] ) {
				$sub_heading_style_inline .= 'color:' . esc_attr( $ult_heading_settings['sub_heading_color'] ) . ';';
			}
			// attach margins for sub heading.
			if ( '' != $ult_heading_settings['sub_heading_margin'] ) {
				$sub_heading_style_inline .= esc_attr( $ult_heading_settings['sub_heading_margin'] );
			}
			...
			if ( 'line_with_icon' == $ult_heading_settings['spacer'] ) {
				...
				if ( 'selector' == $ult_heading_settings['icon_type'] ) {
					$wrapper_style .= 'height:' . esc_attr( $ult_heading_settings['icon_size'] ) . 'px;';
				}
				$icon_style_inline = 'font-size:' . esc_attr( $ult_heading_settings['icon_size'] ) . 'px;';
			} elseif ( 'line_only' == $ult_heading_settings['spacer'] ) {
				$wrap_width         = $ult_heading_settings['line_width'];
				$line_style_inline  = 'border-style:' . esc_attr( $ult_heading_settings['line_style'] ) . ';';
				$line_style_inline .= 'border-bottom-width:' . esc_attr( $ult_heading_settings['line_height'] ) . 'px;';
				$line_style_inline .= 'border-color:' . esc_attr( $ult_heading_settings['line_color'] ) . ';';
				$line_style_inline .= 'width:' . esc_attr( $wrap_width ) . 'px;';
				$wrapper_style     .= 'height:' . esc_attr( $ult_heading_settings['line_height'] ) . 'px;';
				...
				if ( '' !== $ult_heading_settings['spacer_img_width'] ) {
					$spacer_inline = 'width:' . esc_attr( $ult_heading_settings['spacer_img_width'] ) . 'px';
				}
			...
			$args                    = array(
				'target'      => '.uvc-heading.' . $id . ' ' . esc_attr( $ult_heading_settings['heading_tag'] ),
				'media_sizes' => array(
			...
			if ( '' != $ult_heading_settings['main_heading'] ) {
				$output .= '<div class="uvc-main-heading ult-responsive" ' . $main_heading_responsive . '><' . esc_attr( $ult_heading_settings['heading_tag'] ) . ' style="' . esc_attr( $main_heading_style_inline ) . '">' . wp_kses_post( $ult_heading_settings['main_heading'] ) . '</' . esc_attr( $ult_heading_settings['heading_tag'] ) . '></div>';
			}
		...
		public function ultimate_heading_spacer( $wrapper_class, $wrapper_style, $icon_inline ) {
			$ult_heading_settings['spacer'] = '<div class="uvc-heading-spacer ' . esc_attr( $wrapper_class ) . '" style="' . esc_attr( $wrapper_style ) . '">' . $icon_inline . '</div>';
