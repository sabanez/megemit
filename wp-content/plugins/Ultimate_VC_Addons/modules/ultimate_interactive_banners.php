...
if ( ! class_exists( 'Ultimate_VC_Addons_Interactive_Banners' ) ) {
	...
	class Ultimate_VC_Addons_Interactive_Banners {
		...
		public function banner_shortcode( array $atts ) {
			...
			if ( ! in_array( $utl_interactive_banner_settings['heading_tag'], $valid_tags ) ) {
				$utl_interactive_banner_settings['heading_tag'] = 'h3';
			}
			...
			if ('' === $img ) {
				$output .= "\n\t" . '<div class=" ' . esc_attr( $utl_interactive_banner_settings['banner_opacity'] ) . '-background" style="' . esc_attr( $utl_interactive_banner_settings['banner_overlay_bg_color'] ) . '">';
			}
			else {
			...
			if ( 'with_description' == $utl_interactive_banner_settings['icon_disp'] || 'both' == $utl_interactive_banner_settings['icon_disp'] ) {
				if ( '' !== $utl_interactive_banner_settings['banner_icon'] ) {
					$output .= "\n\t\t" . '<div class="bb-back-icon">' . wp_kses_post( $icon ) . '</div>';
