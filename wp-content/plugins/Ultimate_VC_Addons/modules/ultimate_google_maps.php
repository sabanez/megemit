					...
					array(
						...
						'params'                  => array(
							...
							array(
								...
								'value'       => '',
								'description' => "<a target='_blank' rel='noopener' href='https://mapstyle.withgoogle.com/'>" . __( 'Click here', 'ultimate_vc' ) . '</a> ' . __( 'to get the style JSON code for styling your map.', 'ultimate_vc' ),
								'group'       => 'Styling',
							),
							array(
								'type'        => 'textfield',
		...
		public function display_ultimate_map( $atts, $content = null ) {
			...
			try
			{
				...
				var mapOptions=
				{
					...
					scaleControl: true,
					streetViewControl: " . ( esc_js( $ult_google_settings['streetviewcontrol'] ) ) . ',
			...
			if ( '' == $ult_google_settings['map_style'] ) {
				...
				$output .= '};';
			if ( '' !== $ult_google_settings['map_style'] ) {
				$map_style = esc_js( wp_strip_all_tags( $ult_google_settings['map_style'] ) );
				$output   .= 'var styles = ' . rawurldecode( base64_decode( ( $map_style ) ) ) . '; var styledMap = new google.maps.StyledMapType(styles,{name: "Styled Map"});'; //phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode
			}
			...
			if ( '' != $marker_lat && '' != $marker_lng ) {
				...
				if ( '' !== trim( $content ) ) {
					$output .= "var infowindow = new google.maps.InfoWindow();
							infowindow.setContent('<div class=\"map_info_text\" style=\'color:#000;\'>" . wp_kses_post( trim( preg_replace( '/\s+/', ' ', do_shortcode( $content ) ) ) ) . "</div>');";
