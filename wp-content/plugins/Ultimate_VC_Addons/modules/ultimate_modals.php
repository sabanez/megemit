	...
	class Ultimate_VC_Addons_Modals {
		...
		public function uavc_is_medium_device() {
			...
			if ( empty( $_SERVER['HTTP_USER_AGENT'] ) ) {  
				$is_medium = false;
			} elseif ( strpos( sanitize_text_field( $_SERVER['HTTP_USER_AGENT'] ), 'iPad' ) !== false ) { // PHPCS:Ignore WordPressVIPMinimum.Variables.RestrictedVariables.cache_constraints___SERVER__HTTP_USER_AGENT__
				$is_medium = true;
			} else {
				$is_medium = false;
			}
		...
		public function modal_shortcode( $atts, $content = null ) {
			...
			if ( '' !== $ult_modal_box_settings['content_bg_color'] ) {
				$content_style .= 'background:' . esc_attr( $ult_modal_box_settings['content_bg_color'] ) . ';';
			}
			// Create style for content text color.
			if ( '' !== $ult_modal_box_settings['content_text_color'] ) {
				$content_style .= 'color:' . esc_attr( $ult_modal_box_settings['content_text_color'] ) . ';';
			}
			...
			if ( '' !== $ult_modal_box_settings['header_bg_color'] ) {
				$header_style .= 'background:' . esc_attr( $ult_modal_box_settings['header_bg_color'] ) . ';';
			}
			// Create style for header text color.
			if ( '' !== $ult_modal_box_settings['header_text_color'] ) {
				$header_style .= 'color:' . esc_attr( $ult_modal_box_settings['header_text_color'] ) . ';';
			}
			...
			if ( '' !== $ult_modal_box_settings['modal_border_style'] ) {
				$border_style .= 'border-style:' . esc_attr( $ult_modal_box_settings['modal_border_style'] ) . ';';
				$border_style .= 'border-width:' . esc_attr( $ult_modal_box_settings['modal_border_width'] ) . 'px;';
				$border_style .= 'border-radius:' . esc_attr( $ult_modal_box_settings['modal_border_radius'] ) . 'px;';
				$border_style .= 'border-color:' . esc_attr( $ult_modal_box_settings['modal_border_color'] ) . ';';
				$header_style .= 'border-color:' . esc_attr( $ult_modal_box_settings['modal_border_color'] ) . ';';
			}
			// Check if overlay_bg_opacity exists and is not empty.
			if ( ! isset( $ult_modal_box_settings['overlay_bg_opacity'] ) || '' === $ult_modal_box_settings['overlay_bg_opacity'] ) {
				$ult_modal_box_settings['overlay_bg_opacity'] = 1;
			} else {
				$ult_modal_box_settings['overlay_bg_opacity'] /= 100;
			}
			...
			if ( 'button' == $ult_modal_box_settings['modal_on'] ) {
				if ( '' !== $ult_modal_box_settings['btn_bg_color'] ) {
					$style .= 'background:' . esc_attr( $ult_modal_box_settings['btn_bg_color'] ) . ';';
					$style .= 'border-color:' . esc_attr( $ult_modal_box_settings['btn_bg_color'] ) . ';';
				}
				if ( '' !== $ult_modal_box_settings['btn_txt_color'] ) {
					$style .= 'color:' . esc_attr( $ult_modal_box_settings['btn_txt_color'] ) . ';';
				}
				...
				if ( '' !== $ult_modal_box_settings['btn_img'] ) {
					...
					$html   .= '<img tabindex="0" src="' . esc_url( apply_filters( 'ultimate_images', $img ) ) . '" alt="' . esc_attr( $btn_alt ) . '" data-class-id="content-' . esc_attr( $uniq ) . '" class="ult-modal-img ' . esc_attr( $modal_class ) . ' ult-align-' . esc_attr( $ult_modal_box_settings['modal_on_align'] ) . ' ult-modal-image-' . esc_attr( $ult_modal_box_settings['el_class'] ) . '" ' . $modal_data_class . '/>';
				}
				...
				if ( '' !== $ult_modal_box_settings['txt_color'] ) {
					$style .= 'color:' . esc_attr( $ult_modal_box_settings['txt_color'] ) . ';';
					$style .= 'cursor:pointer;';
				}
				if ( '' != $ult_modal_box_settings['el_class'] ) {
					$modal_class .= ' ' . $ult_modal_box_settings['el_class'] . '-link ';
				}
				$html .= '<span tabindex="0" ' . $madal_trg_data_list . ' style="' . esc_attr( $style ) . ' ' . esc_attr( $trigger_text_style ) . '" data-class-id="content-' . esc_attr( $uniq ) . '" class="' . esc_attr( $modal_class ) . ' ult-responsive mycust ult-align-' . esc_attr( $ult_modal_box_settings['modal_on_align'] ) . '" ' . $modal_data_class . '>' . wp_kses_post( $ult_modal_box_settings['read_text'] ) . '</span>';
			}
			$html .= '</div>';
			...
			if ( '' != $ult_modal_box_settings['img_close_background_color'] ) {
				$img_background_color = 'background-color: ' . esc_attr( $ult_modal_box_settings['img_close_background_color'] ) . ';border-radius: 50%;padding:' . $custom_padding_img . 'px;box-sizing: content-box;';
			}
					...
					array(
						...
						'params'                  => array(
							...
							array(
								...
								'description' => __(
									"Please put the embed code in the content for videos, <br>
