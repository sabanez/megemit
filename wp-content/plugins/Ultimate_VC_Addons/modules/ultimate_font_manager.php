...
if ( ! class_exists( 'Ultimate_VC_Addons_Google_Font_Manager' ) ) {
	...
	class Ultimate_VC_Addons_Google_Font_Manager {
					...
					$font_call .= $sfont['font_family'];
                                       if ( isset( $sfont['variants'] ) ) :
                                               ...
                                               if ( ! is_array( $variants ) ) {
                                                       $variants = json_decode( stripslashes( (string) $variants ), true );
                                               }
                                               if ( ! is_array( $variants ) ) {
                                                       $variants = array();
                                               }
                                               if ( ! empty( $variants ) ) {
                                                       ...
                                                       foreach ( $variants as $variant ) {
                                                               if ( is_array( $variant ) ) {
                                                                       $variant_selected = isset( $variant['variant_selected'] ) ? $variant['variant_selected'] : 'true';
                                                                       $variant_value    = isset( $variant['variant_value'] ) ? $variant['variant_value'] : '';
                                                               } else {
                                                       ...
                                                       foreach ( $variants_array as $vkey => $variant_val ) {
                                                               $font_call .= $variant_val;
                                       ...
                                       if ( ! empty( $sfont['subsets'] ) ) {
                                               $subset_array = array();
                                               $subsets = $sfont['subsets'];
                                               if ( ! is_array( $subsets ) ) {
                                                       $subsets = json_decode( stripslashes( (string) $subsets ), true );
                                               }
                                               if ( ! is_array( $subsets ) ) {
                                                       $subsets = array();
                                               }

                                               foreach ( $subsets as $tsubset ) {
                                                       if ( is_array( $tsubset ) ) {
                                                               $subset_selected = isset( $tsubset['subset_selected'] ) ? $tsubset['subset_selected'] : 'true';
                                                               $subset_value    = isset( $tsubset['subset_value'] ) ? $tsubset['subset_value'] : '';
                                                       } else {
                                                               $subset_selected = 'true';
                                                               $subset_value    = $tsubset;
                                                       }

                                                       if ( 'true' == $subset_selected || true == $subset_selected ) {
                                                               $subset_main_array[] = $subset_value;
                                                       }
                                               }
                                        }
				}
		...
		<div class="wrap uavc-gfont">
			<h2>
				...
				<div class="fonts-selected-list">
					<h3><?php echo esc_html__( 'Your Font Collection', 'ultimate_vc' ); ?></h3>
										...
										if ( ! empty( $sfont['variants'] ) || ! empty( $sfont['subsets'] ) ) :
											...
											if ( ! empty( $sfont['variants'] ) ) :
                                               ...
                                               foreach ( $sfont['variants'] as $svkey => $svariants ) {
                                                       if ( ! is_array( $svariants ) ) {
                                                               $svariants = array(
                                                                       'variant_value'    => $svariants,
                                                                       'variant_selected' => 'true',
                                                               );
                                                       }
		...
		public function refresh_google_fonts_list() {
			...
			if ( empty( $fonts ) ) {
				try {
					$fonts = wp_remote_get( 'https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyD_6TR2RyX2VRf8bABDRXCcVqdMXB5FQvs' ); // @codingStandardsIgnoreLine.
			...
			if ( true != $error || 0 == count( $fonts ) ) {
				...
				$google_font_count = count( is_countable( $google_fonts ) ? $google_fonts : array() ); // PHPCS:Ignore PHPCompatibility.FunctionUse.NewFunctions.is_countableFound
		...
		public function get_google_fonts_list() {
			...
			if ( ! empty( $google_fonts ) ) :
				...
				$start_count      = isset( $_POST['start'] ) ? sanitize_text_field( $_POST['start'] ) : '';
				$fetch_count      = isset( $_POST['fetch'] ) ? sanitize_text_field( $_POST['fetch'] ) : '';
			...
			$response['fonts_count'] = count( is_countable( $google_fonts ) ? $google_fonts : array() ); // PHPCS:Ignore PHPCompatibility.FunctionUse.NewFunctions.is_countableFound
			if ( '' != $search ) {
		...
		public function add_selected_google_font() {
			...
			$font_name   = isset( $_POST['font_name'] ) ? sanitize_text_field( $_POST['font_name'] ) : '';
                       $variants    = isset( $_POST['variants'] ) ? $_POST['variants'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
                       $subsets     = isset( $_POST['subsets'] ) ? $_POST['subsets'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
                       ...
                       foreach ( $variants as &$variant ) {
                               $variant = array(
                                       'variant_value'    => isset( $variant['variant_value'] ) ? sanitize_text_field( $variant['variant_value'] ) : '',
                                       'variant_selected' => ( isset( $variant['variant_selected'] ) && filter_var( $variant['variant_selected'], FILTER_VALIDATE_BOOLEAN ) ) ? 'true' : 'false',
                               );
                       }
                       unset( $variant );
                       ...
                       foreach ( $subsets as &$subset ) {
                               $subset = array(
                                       'subset_value'    => isset( $subset['subset_value'] ) ? sanitize_text_field( $subset['subset_value'] ) : '',
                                       'subset_selected' => ( isset( $subset['subset_selected'] ) && filter_var( $subset['subset_selected'], FILTER_VALIDATE_BOOLEAN ) ) ? 'true' : 'false',
                               );
                       }
                       unset( $subset );
