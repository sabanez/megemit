...
if ( ! class_exists( 'Ultimate_VC_Addons_Team' ) ) {
	...
	class Ultimate_VC_Addons_Team {
		...
		public function ult_team_shortcode( $atts, $content ) {
			...
			// Ensure team_member_name_tag is a valid HTML tag.
			$valid_tags = array( 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'div', 'p', 'span' );

			// Dual-color Heading tag.
			if ( ! in_array( $ult_team_settings['team_member_name_tag'], $valid_tags ) ) {
				$ult_team_settings['team_member_name_tag'] = 'h2';
			}
			...
			$ult_team_settings['team_img_opacity']              = ( isset( $ult_team_settings['team_img_opacity'] ) && trim( $ult_team_settings['team_img_opacity'] ) !== '' ) ? sanitize_text_field( $ult_team_settings['team_img_opacity'] ) : '1';
			$ult_team_settings['team_img_hover_opacity']        = ( isset( $ult_team_settings['team_img_hover_opacity'] ) && trim( $ult_team_settings['team_img_hover_opacity'] ) !== '' ) ? sanitize_text_field( $ult_team_settings['team_img_hover_opacity'] ) : '0.65';
			$ult_team_settings['team_img_hover_opacity_style3'] = ( isset( $ult_team_settings['team_img_hover_opacity_style3'] ) && trim( $ult_team_settings['team_img_hover_opacity_style3'] ) !== '' ) ? sanitize_text_field( $ult_team_settings['team_img_hover_opacity_style3'] ) : '0.1';
			...
			$ult_team_settings['team_member_style'] = ( isset( $ult_team_settings['team_member_style'] ) && trim( $ult_team_settings['team_member_style'] ) !== '' ) ? sanitize_text_field( $ult_team_settings['team_member_style'] ) : 'style-1';
			$ult_team_settings['custom_team_class'] = ( isset( $ult_team_settings['custom_team_class'] ) && trim( $ult_team_settings['custom_team_class'] ) !== '' ) ? sanitize_text_field( $ult_team_settings['custom_team_class'] ) : '';

			// Set responsive width.
			$ult_team_settings['team_responsive_width'] = ( isset( $ult_team_settings['team_responsive_width'] ) && trim( $ult_team_settings['team_responsive_width'] ) !== '' ) ? sanitize_text_field( $ult_team_settings['team_responsive_width'] ) : '';
			$ult_team_settings['team_responsive_width'] = ( isset( $ult_team_settings['team_member_responsive_enable'] ) && trim( $ult_team_settings['team_member_responsive_enable'] ) == 'on' ) ? sanitize_text_field( $ult_team_settings['team_responsive_width'] ) : '';
			...
			$ult_team_settings['team_member_name_tag'] = ( isset( $ult_team_settings['team_member_name_tag'] ) && trim( $ult_team_settings['team_member_name_tag'] ) !== '' ) ? esc_html( $ult_team_settings['team_member_name_tag'] ) : 'h2';
			...
			// title box style.
			$title_box_style  = '';
			$title_box_style .= trim( $ult_team_settings['title_box_padding'] ) != '' ? sanitize_text_field( $ult_team_settings['title_box_padding'] ) : '';
			...
			$team_name_args              = array(
				...
				'media_sizes' => array(
					'font-size'   => esc_attr( $ult_team_settings['team_member_name_font_size'] ),
					'line-height' => esc_attr( $ult_team_settings['team_member_name_line_height'] ),
				),
			);
			...
			if ( 'style-3' == $ult_team_settings['team_member_style'] ) {
				...
				$team_desc_args              = array(
					...
					'media_sizes' => array(
						'font-size'   => esc_attr( $ult_team_settings['team_member_description_font_size'] ),
						'line-height' => esc_attr( $ult_team_settings['team_member_description_line_height'] ),
					),
				);
				$team_member_desc_responsive = get_ultimate_vc_responsive_media_css( $team_desc_args );
				...
				echo '<' . esc_html( $ult_team_settings['team_member_name_tag'] ) . ' class="ult-team-member-name ult-responsive" ' . $team_member_name_responsive . ' style="' . esc_attr( $team_member_name_font_styling ) . '">' . esc_html( $ult_team_settings['name'] ) . '</' . esc_html( $ult_team_settings['team_member_name_tag'] ) . '>'; // PHPCS:ignore:WordPress.Security.EscapeOutput.OutputNotEscaped
				if ( 'on' == $ult_team_settings['link_switch'] && '' != $ult_team_settings['staff_link'] ) {
					echo '</a>';
				}
				if ( $ult_team_settings['pos_in_org'] ) {
					echo '<div class="ult-team-member-position ult-responsive" ' . $team_member_position_responsive . ' style="' . esc_attr( $team_member_position_font_styling ) . '">' . esc_html( $ult_team_settings['pos_in_org'] ) . '</div>'; // PHPCS:ignore:WordPress.Security.EscapeOutput.OutputNotEscaped
				}
				...
				if ( 'on' == $ult_team_settings['divider_effect'] ) {
					...
					if ( 'center' != $ult_team_settings['team_member_align_style'] ) {
						$divider_margin = 'margin-' . esc_attr( $ult_team_settings['team_member_align_style'] ) . ':0px';
					}
				...
				echo '<' . esc_attr( $ult_team_settings['team_member_name_tag'] ) . ' class="ult-team-member-name ult-responsive" ' . $team_member_name_responsive . ' style="' . esc_attr( $team_member_name_font_styling ) . '">' . esc_html( $ult_team_settings['name'] ) . '</' . esc_attr( $ult_team_settings['team_member_name_tag'] ) . '>'; // PHPCS:ignore:WordPress.Security.EscapeOutput.OutputNotEscaped
				if ( $ult_team_settings['pos_in_org'] ) {
					echo '<div class="ult-team-member-position ult-responsive" ' . $team_member_position_responsive . ' style="' . esc_attr( $team_member_position_font_styling ) . '">' . esc_html( $ult_team_settings['pos_in_org'] ) . '</div>'; // PHPCS:ignore:WordPress.Security.EscapeOutput.OutputNotEscaped
				}
				...
				if ( $social_icons && count( $social_icons ) > 0 && 'on' == $ult_team_settings['social_icon_effect'] ) {
					...
					foreach ( $social_icons as $social_link ) {
						...
						if ( isset( $social_link->selected_team_icon ) && '' !== $social_link->selected_team_icon ) {
							...
							$social_icon_url         = ( isset( $social_link->social_icon_url ) && '' !== $social_link->social_icon_url ) ? esc_url( $social_link->social_icon_url ) : '#';
							$social_link_title       = ( isset( $social_link->social_link_title ) && '' !== $social_link->social_link_title ) ? esc_attr( $social_link->social_link_title ) : '';
							$social_icon_color       = ( isset( $social_link->social_icon_color ) && '' !== $social_link->social_icon_color ) ? esc_attr( $social_link->social_icon_color ) : 'inherit';
							$default_icon_color      = ( 'inherit' != $social_icon_color ) ? 'color:' . esc_attr( $social_icon_color ) . ';' : '';
							$social_icon_hover_color = ( isset( $social_link->social_icon_hover_color ) && '' !== $social_link->social_icon_hover_color ) ? esc_attr( $social_link->social_icon_hover_color ) : 'inherit';
							echo "<a href='" . esc_url( $social_icon_url ) . "' target='_blank' rel='noopener' title='" . esc_attr( $social_link_title ) . "' class='ult-team ult-social-icon' style='" . esc_attr( $icon_styling ) . '; ' . esc_attr( $default_icon_color ) . "'  data-iconcolor='" . esc_attr( $social_icon_color ) . "' data-iconhover='" . esc_attr( $social_icon_hover_color ) . "' ><i class='" . esc_attr( $social_link->selected_team_icon ) . "'></i></a>";
						}
					}
			...
			if ( $is_preset && current_user_can( 'manage_options' ) ) {
				$text = 'array ( ';
					...
					array(
						...
						'params'           => array(
							...
							array(
								...
								'value'            => array(
									esc_html__( 'Default', 'ultimate_vc' )  => 'h2',
									esc_html__( 'H1', 'ultimate_vc' )  => 'h1',
									esc_html__( 'H3', 'ultimate_vc' )  => 'h3',
