...
if ( ! function_exists( 'ult_info_table_generate_design01' ) ) {
	...
	function ult_info_table_generate_design01( $atts, $content = null ) {
		...
		if ( ! in_array( $ult_info_design01['heading_tag'], $valid_tags ) ) {
			$ult_info_design01['heading_tag'] = 'h3';
		}

		// Dual-color Heading tag.
		if ( ! in_array( $ult_info_design01['sub_heading_tag'], $valid_tags ) ) {
			$ult_info_design01['sub_heading_tag'] = 'h5';
		}
		...
		if ( is_numeric( $ult_info_design01['heading_line_height'] ) ) {
			...
			$info_table_sub_head_args      = array(
				'target'      => '#' . $info_table_id . ' ' . esc_attr( $ult_info_design01['sub_heading_tag'] ), // set targeted element e.g. unique class/id etc.
				'media_sizes' => array(
			...
			$output .= '<div class="ult_pricing_heading" id="' . esc_attr( $info_table_id ) . '">
							<' . esc_attr( $ult_info_design01['heading_tag'] ) . ' class="ult-responsive" ' . $info_table_data_list . ' style="' . esc_attr( $heading_style_inline ) . '">' . esc_html( $ult_info_design01['package_heading'] ) . '</' . esc_attr( $ult_info_design01['heading_tag'] ) . '>';
			if ( '' !== $ult_info_design01['package_sub_heading'] ) {
				$output .= '<' . esc_attr( $ult_info_design01['sub_heading_tag'] ) . ' class="ult-responsive" ' . $info_table_sub_head_data_list . 'style="' . esc_attr( $sub_heading_inline ) . '">' . esc_html( $ult_info_design01['package_sub_heading'] ) . '</' . esc_attr( $ult_info_design01['sub_heading_tag'] ) . '>';
			}
			...
			if ( 'true' == $ult_info_design01['use_cta_btn'] ) {
				$output .= '<div id="' . esc_attr( $info_table_btn_id ) . '" class="ult_price_link" style="' . esc_attr( $normal_style ) . '">
							<a ' . Ultimate_VC_Addons::uavc_link_init( $link, $target, $link_title, $rel ) . ' ' . $info_table_btn_data_list . ' class="ult-responsive ult_price_action_button" style="' . esc_attr( $featured_style ) . ' ' . esc_attr( $button_inline ) . '">' . esc_html( $ult_info_design01['package_btn_text'] ) . '</a>
						</div><!--ult_price_link-->';
			}
