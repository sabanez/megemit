		...
		public function woocomposer_grid_shortcode( $atts ) {
			...
			if ( $product_categories ) {
				...
				foreach ( $product_categories as $category ) {
					...
					?>
					">
						<div class="wcmp-product wcmp-img-<?php echo esc_attr( $ult_woocomposer_cat_grid_shortcode['img_animate'] ); ?> wcmp-cat-<?php echo esc_attr( $ult_woocomposer_cat_grid_shortcode['design_style'] ) . ' animated ' . esc_attr( $ult_woocomposer_cat_grid_shortcode['product_animation'] ); ?>" style="<?php echo esc_attr( $border ); ?>">
						<?php do_action( 'woocommerce_before_subcategory', $category ); ?>
						<a href="<?php echo get_term_link( $category->slug, 'product_cat' ); ?>" style="text-align:<?php echo esc_attr( $ult_woocomposer_cat_grid_shortcode['text_align'] ); ?>;">
							...
							<h3 style="<?php echo esc_attr( $size ); ?>">
								...
								if ( $category->count > 0 ) {
									echo apply_filters( 'woocommerce_subcategory_count_html', ' <mark class="count" style="' . esc_attr( $count_style ) . '">' . $category->count . ' ' . $ult_woocomposer_cat_grid_shortcode['cat_count'] . '</mark>', $category );
								}
								?>
							</h3>
							<?php
