jQuery(document).ready(function ($) {
	...
	$('#form-items .swpm-draggable-form-items').click(function (e) {
		...
		if (field_key in swpmFbVars.uniq_fields && $('#swpm-menu-to-edit').find('li[data-field-key="' + field_key + '"]').length !== 0) {
			alert(swpmFbVars.uniq_fields[field_key].msg);
			return false;
		}

		$('#form-spinner').addClass('is-active');
		$.post(ajaxurl,
			{
			...
			$('#form-spinner').removeClass('is-active');
	...
	// !Sticky sidebar
	if ($('.columns-2 #side-sortables').length && $('#swpm_form_items_meta_box').length) {
