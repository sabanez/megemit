...
class SwpmFbFormsList extends WP_List_Table {
    ...
    function get_columns() {
        $columns = array(
            ...
            'form_title' => __('Custom Form Name', 'swpm-form-builder'),
    ...
    function prepare_items() {
        ...
        foreach ($search_terms as $term) {
            ...
            $search .= "{$searchand}((forms.form_title LIKE '%{$term}%') OR (forms.form_key LIKE '%{$term}%') OR (forms.form_notification_subject LIKE '%{$term}%'))";
            $searchand = ' AND ';
        }
