...
define( 'EBSSO_MOODLE_PLUGIN_LINK', 'https://edwiser.org/wp-content/uploads/edd/2018/09/wdmwpmoodle.zip' );
...
function pluginUpdateHook() {
	...
	if ( ! $oldVersion || version_compare( '1.4.4', $oldVersion ) == 1 ) {
		require_once plugin_dir_path( __FILE__ ) . 'includes/class-sso-activator.php';
		ebsso\SsoActivator::activate( false );
		// Save plugins current version in DB.
		update_option( 'eb_sso_version', '1.4.4' );
	}
}
...
require_once 'includes/class-single-sign-on.php';
