        ...
        try {
            ...
            if (isset($_SESSION['token'])) {
                try {
                    ...
                    $this->ebLogger->logger()->add("SSO Log: Google OAuth login access token exception" . serialize($e->getMessage()));
                }
            }
            if ($this->googleClient->getAccessToken()) {
                try {
                    ...
                    $this->ebLogger->logger()->add("SSO Log: Google OAuth login failed to fetch google OAuth profile data." . serialize($e->getMessage()));
                    auth_redirect();
                    exit();
                }
            ...
            $this->ebLogger->logger()->add("SSO Log: Google OAuth login failed with exception: " . serialize($e->getMessage()));
        }
    }
