...
class SsoGetData
{
    ...
    public static function getDataFromDb($wdm_plugin_data)
    {
        ...
        if (! $get_trans) {
            ...
            if ($license_key) {
                ...
                $curRespCode = wp_remote_retrieve_response_code($response);

                if ($license_data == null || ! in_array($curRespCode, $valid_response_code)) {
                ...
                $active_site = SsoGetData::getSiteList($plugin_slug);
    ...
    public static function setResponseData($license_status, $active_site, $plugin_slug, $set_transient = false)
    {
        ...
        if ($license_status == 'expired' && ( ! empty($active_site) || $active_site != "")) {
            self::$responseData = 'unavailable';
        } elseif ($license_status == 'expired' || $license_status == 'valid') {
