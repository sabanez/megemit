...
class SingleSignOn
{
    ...
    public function runPluginUpdateHook()
    {
         ...
         */
        if ($oldVersion == false || (version_compare($this->version, '1.3.8') == -1 && version_compare($oldVersion, '1.3.0') == -1 )) {
            /*
    ...
    public function eb_login_from_course_link() {
        if ( isset( $_GET['mdl_course_id'] ) ) {
            ...
            if ( $user->ID ) {
                ...
                $mdlLogout->mdlLoggedIn( $user->user_login, $user );
            }
        }
    }
    ...
    public function verifyKey()
    {
        ...
        if (is_wp_error($response)) {
            ...
            } elseif (isset($body->exception) && $body->exception == "dml_missing_record_exception" ) {
                wp_send_json_error(__('Please check if you have Moodle Single Sign On Plugin installed !', "single_sign_on_text_domain"));
            }
            
            if ($body->success == true) {
                wp_send_json_success( __( 'Token verified successfully !', 'single_sign_on_text_domain' ) );
            } else {
                $msg = $body->msg;
            }
        } else {
        ...
        if ( isset( $post ) && has_shortcode( $post->post_content, 'wdm_generate_link' ) ) {
            if ( is_user_logged_in() ) {
                ...
                if ( $moodle_user_id ) {
                    ...
                    $user         = wp_get_current_user();
                    $mdlLogout    = new SsoManageMooLogin( $this->plugin_name, $this->version );
                    $mdlLogout->mdlLoggedIn( $user->user_login, $user, '', $redirect_url );
    ...
    public function courseAccessButton($access_button, $access_params)
    {
        ...
        /*$url = get_site_url() . '/wp-json/edwiser-bridge/sso/?mdl_course_id=' . $moodle_course_id;
        return $url;*/
    ...
    public function mdlTriggeredAction()
    {
        ...
        if ( isset( $_GET['wdmaction'] ) && isset( $_GET['mdl_uid'] ) && isset( $_GET['verify_code'] ) ) {
            $setting = get_option('eb_sso_settings_general');
            $redirect_to = eb_get_mdl_url();

            // Get stored data.
            $wp_user_id      = eb_get_wp_user_id_from_moodle_id( $_GET['mdl_uid'] );
            ...
            if ( $wp_user_id ) {
                ...
                // verify key.
                if ( isset( $decrypted_args ) && getKeyValue( $decrypted_args, 'mdl_one_time_code' ) == $_GET['verify_code'] ) {
        ...
        } elseif( isset( $_GET['wdmaction'] ) && isset( $_GET['data'] ) ){
            $connOptions = get_option('eb_connection');
            $mdlUrl      = $this->getMdlConnectionUrl($connOptions);
            ...
            if ( is_user_logged_in() ) {
                ...
                $decrypted_args = getDecryptedQueryArgs( $_GET['data'], $wp_key );
                ...
                if ( getKeyValue( $decrypted_args, 'mdl_key' ) == $wp_key ) {
                    ...
                    if ( $user->ID ) {
                        $mdlLogout   = new SsoManageMooLogin($this->plugin_name, $this->version);
                        $mdlLogout->mdlLoggedIn( $user->user_login, $user, '', $mdlUrl );
                    }
                }
            } else {
                // If not logged in then redirect to the Wordpress login page.
                wp_redirect( add_query_arg( array( 'redirect_to' => $mdlUrl, 'login_action' => 'moodle' ), get_site_url() . '/wp-login.php' ) );
            }
        }
    }
    ...
    public function external_api_endpoint_def( $request_data ) {
        ...
        if ( isset( $_POST['wdmargs'] ) ) {
            $setting = get_option('eb_sso_settings_general');
            $wp_key  = isset($setting['eb_sso_secret_key']) ? (string) $setting['eb_sso_secret_key'] : '';

            $decrypted_args = getDecryptedQueryArgs($_POST['wdmargs'], $wp_key);
            $mdl_key        = (string) getKeyValue($decrypted_args, 'mdl_key');
            $mdl_uid        = getKeyValue($decrypted_args, 'mdl_uid');
            $mdl_email      = getKeyValue($decrypted_args, 'mdl_email');
            ...
            if ($mdl_key === $wp_key) {
                ...
                if(isset($user->ID) && !empty($user->ID)) {
                    update_user_meta( $user->ID, 'eb_sso_user_login_logout_session', $_POST['wdmargs'] );
                }
            }
        }
    }
