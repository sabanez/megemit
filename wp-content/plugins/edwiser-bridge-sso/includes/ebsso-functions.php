...
if (!function_exists('\ebsso\createEbUser')) {
    ...
    function createEbUser($userData)
    {
        ...
        $user_id = $ebUserMang->createWordpressUser( $userData['email'], $first_name, $last_name );
    }
}
...
function getRedirectUrl($get, $post_content, $redirect_url = '')
{
    ...
    if (isset($post_content) && has_shortcode($post_content, 'woocommerce_my_account') && ! isset( $_GET['mdl_course_id'] ) ) {
        $redirect_url = $_SERVER['HTTP_REFERER'];
